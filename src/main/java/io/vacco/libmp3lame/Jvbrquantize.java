...
final class Jvbrquantize {
  ...
  */
  ...
  private static final class Jalgo_s {
    ...
    private Jlame_internal_flags gfc;
    private Jgr_info cod_info;
  ...
  */
  ...
  private static final float vec_max_c(final float[] xr34, int xoffset, final int bw) {
    ...
    switch (remaining) {
      case 3:
        if (xfsf < xr34[xoffset + 2]) {
          xfsf = xr34[xoffset + 2];
        }
      case 2:
        if (xfsf < xr34[xoffset + 1]) {
          xfsf = xr34[xoffset + 1];
        }
      case 1:
        if (xfsf < xr34[xoffset + 0]) {
          xfsf = xr34[xoffset + 0];
        }
      default:
        break;
  ...
  private static final int find_lowest_scalefac(final float xr34) {
    int sf_ok = 255;
    int sf = 128, delsf = 64;
    final float ixmax_val = Jquantize_pvt.IXMAX_VAL;
    for (int i = 0; i < 8; ++i) {
      final float xfsf = Jquantize_pvt.ipow20[sf] * xr34;
      if (xfsf <= ixmax_val) {
        sf_ok = sf;
        sf -= delsf;
      } else {
  ...
  private static final void k_34_4(final float x[] /*[4]*/, final int l3[] /*[4]*/, int l3offset) {
    /* if( Jquantize_pvt.TAKEHIRO_IEEE754_HACK ) {
    		x[0] += MAGIC_FLOAT;
    		float fi0 = x[0];
    		x[1] += MAGIC_FLOAT;
    		float fi1 = x[1];
    		...
    		x[3] += MAGIC_FLOAT;
    		float fi3 = x[3];
    		fi0 = x[0] + Jquantize_pvt.adj43[ Float.floatToIntBits( fi0 ) - MAGIC_INT ];
    		fi1 = x[1] + Jquantize_pvt.adj43[ Float.floatToIntBits( fi1 ) - MAGIC_INT ];
    		fi2 = x[2] + Jquantize_pvt.adj43[ Float.floatToIntBits( fi2 ) - MAGIC_INT ];
    		fi3 = x[3] + Jquantize_pvt.adj43[ Float.floatToIntBits( fi3 ) - MAGIC_INT ];
    ...
    l3[++l3offset] = (int) x[2];
    l3[++l3offset] = (int) x[3];
    x[3] += Jquantize_pvt.adj43[l3[l3offset]];
    x[2] += Jquantize_pvt.adj43[l3[--l3offset]];
    x[1] += Jquantize_pvt.adj43[l3[--l3offset]];
    x[0] += Jquantize_pvt.adj43[l3[--l3offset]];
    l3[l3offset] = (int) x[0];
    l3[++l3offset] = (int) x[1];
    l3[++l3offset] = (int) x[2];
    l3[++l3offset] = (int) x[3];
   ...
   */
  private static final float calc_sfb_noise_x34(
      final float[] xr,
      int xroffset,
      final float[] xr34,
      int xr34offset,
    ...
    final float sfpow = Jquantize_pvt.pow20[sf + Jquantize_pvt.Q_MAX2]; /*pow(2.0,sf/4.0 ); */
    final float sfpow34 = Jquantize_pvt.ipow20[sf]; /*pow(sfpow,-3.0/4.0 ); */
    ...
    while (i-- > 0) {
      ...
      final float x0 = v - sfpow * Jquantize_pvt.pow43[l3[0]];
      v = xr[xroffset++];
      if (v < 0) {
        v = -v;
      }
      final float x1 = v - sfpow * Jquantize_pvt.pow43[l3[1]];
      v = xr[xroffset++];
      if (v < 0) {
        v = -v;
      }
      final float x2 = v - sfpow * Jquantize_pvt.pow43[l3[2]];
      v = xr[xroffset++];
      if (v < 0) {
        v = -v;
      }
      final float x3 = v - sfpow * Jquantize_pvt.pow43[l3[3]];
    ...
    if (remaining != 0) {
      ...
      switch (remaining) {
        case 3:
          x[2] = sfpow34 * xr34[xr34offset + 2];
        case 2:
          x[1] = sfpow34 * xr34[xr34offset + 1];
        case 1:
          x[0] = sfpow34 * xr34[xr34offset + 0];
      }
      ...
      switch (remaining) {
        case 3:
          ...
          x2 = v - sfpow * Jquantize_pvt.pow43[l3[2]];
        case 2:
          v = xr[xroffset + 1];
          if (v < 0) {
            v = -v;
          }
          x1 = v - sfpow * Jquantize_pvt.pow43[l3[1]];
        case 1:
          v = xr[xroffset];
          if (v < 0) {
            v = -v;
          }
          x0 = v - sfpow * Jquantize_pvt.pow43[l3[0]];
      }
      xfsf += (x0 * x0 + x1 * x1) + (x2 * x2 + x3 * x3);
    }
    return xfsf;
  ...
  private static final class Jcalc_noise_cache {
    private boolean valid;
    private float value;
  }

  private static final boolean tri_calc_sfb_noise_x34(
      final float[] xr,
      final int xroffset,
      final float[] xr34,
      final int xr34offset,
      final float l3_xmin,
      final int bw,
      final int sf,
      final Jcalc_noise_cache[] did_it) {
    Jcalc_noise_cache nc = did_it[sf]; // java
    if (!nc.valid) {
      nc.valid = true;
  ...
  /** Robert Hegemann 2001-05-01 calculates quantization step size determined by allowed masking */
  ...
  private static final int guess_scalefac_x34(
      final float[] xr,
      final int xroffset,
      final float[] xr34,
      final int xr34offset,
   ...
   */
  private static final int find_scalefac_x34(
      final float[] xr,
      final int xroffset,
      final float[] xr34,
      final int xr34offset,
      ...
      final int bw,
      final int sf_min) {
    final Jcalc_noise_cache did_it[] = new Jcalc_noise_cache[256];
    int sf = 128, sf_ok = 255, delsf = 128, seen_good_one = 0, i;
    i = 256;
    do {
      did_it[--i] = new Jcalc_noise_cache(); // already zeroed
    } while (i > 0);
    for (i = 0; i < 8; ++i) {
      delsf >>= 1;
      if (sf <= sf_min) {
  ...
  private static final int block_sf(
      final Jalgo_s that,
      final float l3_xmin[ /*SFBMAX*/],
      final int vbrsf[ /*SFBMAX*/],
      final int vbrsfmin[ /*SFBMAX*/]) {
    ...
    for (; sfb < Jencoder.SFBMAX; ++sfb) {
      vbrsf[sfb] = maxsf;
      vbrsfmin[sfb] = 0;
    }
    if (m_o > -1) {
      maxsf = m_o;
      for (sfb = 0; sfb < Jencoder.SFBMAX; ++sfb) {
        if (vbrsf[sfb] == 255) {
          vbrsf[sfb] = m_o;
        }
      }
   ...
   *
   ***********************************************************************/
  private static final void quantize_x34(final Jalgo_s that) {
    final float x[] = new float[4];
    final int tmp_l3[] = new int[4];
    final float[] xr34 = that.xr34orig; // java
    int xr34_orig = 0; // xr34[xr34_orig]
    final Jgr_info cod_info = that.cod_info;
    ...
    while (j <= max_nonzero_coeff) {
      final int s =
          (cod_info.scalefac[sfb] + (cod_info.preflag ? Jquantize_pvt.pretab[sfb] : 0)) * ifqstep
              + cod_info.subblock_gain[cod_info.window[sfb]] * 8;
      final int sfac = (cod_info.global_gain - s);
      final float sfpow34 = Jquantize_pvt.ipow20[sfac];
      ...
      if (remaining != 0) {
        ...
        switch (remaining) {
          case 3:
            l3_enc[l3 + 2] = tmp_l3[2];
          case 2:
            l3_enc[l3 + 1] = tmp_l3[1];
          case 1:
            l3_enc[l3 + 0] = tmp_l3[0];
        }
  ...
  private static final byte max_range_short[] = { // [Jencoder.SBMAX_s * 3] = {
    15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15,
    7, 7, 7, 7, 7, 7, 7, 7, 7, 7, 7, 7, 7, 7, 7, 7, 7, 7,
    0, 0, 0
  };

  private static final byte max_range_long[] = { // [Jencoder.SBMAX_l] = {
    15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 7, 7, 7, 7, 7, 7, 7, 7, 7, 7, 0
  };

  private static final byte max_range_long_lsf_pretab[] = { // [Jencoder.SBMAX_l] = {
  ...
  */
  ...
  private static final void set_subblock_gain(
      final Jgr_info cod_info, final int mingain_s[ /*3*/], final int sf[]) {
    ...
    for (int sfb = 0; sfb < Jencoder.SFBMAX; ) {
      sf[sfb++] += sbg0;
      sf[sfb++] += sbg1;
      sf[sfb++] += sbg2;
    }
  ...
  */
  private static final void set_scalefacs(
      final Jgr_info cod_info, final int[] vbrsfmin, final int sf[], final byte[] max_range) {
    final int ifqstep = (cod_info.scalefac_scale == 0) ? 2 : 4;
    final int ifqstepShift = (cod_info.scalefac_scale == 0) ? 1 : 2;
    ...
    if (preflag) {
      for (int sfb = 11; sfb < sfbmax; ++sfb) {
        sf[sfb] += Jquantize_pvt.pretab[sfb] * ifqstep;
      }
    }
    int sfb = 0;
    for (; sfb < sfbmax; ++sfb) {
      final int gain =
          cod_info.global_gain
              - (sbg[window[sfb]] << 3)
              - ((preflag ? Jquantize_pvt.pretab[sfb] : 0) * ifqstep);
  ...
  // #ifndef NDEBUG
  private static final boolean checkScalefactor(
      final Jgr_info cod_info, final int vbrsfmin[ /*SFBMAX*/]) {
    final int ifqstep = cod_info.scalefac_scale == 0 ? 2 : 4;
   ...
   *
   ******************************************************************/
  private static final void short_block_constrain(
      final Jalgo_s that,
      final int vbrsf[ /*SFBMAX*/],
      final int vbrsfmin[ /*SFBMAX*/],
      int vbrmax) {
    final Jgr_info cod_info = that.cod_info;
    final Jlame_internal_flags gfc = that.gfc;
    ...
    {
      final int sf_temp[] = new int[Jencoder.SFBMAX];
      for (int sfb = 0; sfb < Jencoder.SFBMAX; ++sfb) {
   ...
   *
   ******************************************************************/
  private static final void long_block_constrain(
      final Jalgo_s that,
      final int vbrsf[ /*SFBMAX*/],
      final int vbrsfmin[ /*SFBMAX*/],
      int vbrmax) {
    ...
    for (int sfb = 0; sfb < psymax; ++sfb) {
      ...
      final int v0 = v - (v1 << 1);
      v1 = v - (v1 << 2);
      int v1p = max_rangep[sfb] + Jquantize_pvt.pretab[sfb]; // java
    ...
    if (vm0p == 1) {
      ...
      for (int sfb = 0; sfb < psymax; ++sfb) {
        final int a = (gain - vbrsfmin[sfb]) - (Jquantize_pvt.pretab[sfb] << 1);
        if (a <= 0) {
          vm0p = 0;
          vm1p = 0;
          break;
    ...
    if (vm1p == 1) {
      ...
      for (int sfb = 0; sfb < psymax; ++sfb) {
        final int b = (gain - vbrsfmin[sfb]) - (Jquantize_pvt.pretab[sfb] << 2);
        if (b <= 0) {
          vm1p = 0;
          break;
        }
  ...
  private static final void bitcount(final Jalgo_s that) {
    final boolean rc = Jtakehiro.scale_bitcount(that.gfc, that.cod_info);
  ...
  private static final int quantizeAndCountBits(final Jalgo_s that) {
    quantize_x34(that);
    that.cod_info.part2_3_length = Jtakehiro.noquant_count_bits(that.gfc, that.cod_info, null);
    return that.cod_info.part2_3_length;
  }

  private static final int tryGlobalStepsize(
      final Jalgo_s that,
      final int sfwork[ /*SFBMAX*/],
      final int vbrsfmin[ /*SFBMAX*/],
      final int delta) {
    final float xrpow_max = that.cod_info.xrpow_max;
    final int sftemp[] = new int[Jencoder.SFBMAX];
    int vbrmax = 0;
    for (int i = 0; i < Jencoder.SFBMAX; ++i) {
  ...
  private static final void searchGlobalStepsizeMax(
      final Jalgo_s that,
      final int sfwork[ /*SFBMAX*/],
      final int vbrsfmin[ /*SFBMAX*/],
      final int target) {
    ...
    int curr = gain;
    int gain_ok = 1024;
    int nbits = Jtakehiro.LARGE_BITS;
    int l = gain, r = 512;
  ...
  private static final int sfDepth(final int sfwork[ /*SFBMAX*/]) {
    int m = 0;
    for (int j = Jencoder.SFBMAX, i = 0; j > 0; --j, ++i) {
  ...
  private static final void cutDistribution(
      final int sfwork[ /*SFBMAX*/], final int sf_out[ /*SFBMAX*/], final int cut) {
    for (int j = Jencoder.SFBMAX, i = 0; j > 0; --j, ++i) {
  ...
  private static final int flattenDistribution(
      final int sfwork[ /*SFBMAX*/],
      final int sf_out[ /*SFBMAX*/],
      final int dm,
      final int k,
      final int p) {
    int sfmax = 0;
  ...
  private static final int tryThatOne(
      final Jalgo_s that,
      final int sftemp[ /*SFBMAX*/],
      final int vbrsfmin[ /*SFBMAX*/],
      final int vbrmax) {
    final float xrpow_max = that.cod_info.xrpow_max;
    int nbits = Jtakehiro.LARGE_BITS;
  ...
  private static final void outOfBitsStrategy(
      final Jalgo_s that,
      final int sfwork[ /*SFBMAX*/],
      final int vbrsfmin[ /*SFBMAX*/],
      final int target) {
    final int wrk[] = new int[Jencoder.SFBMAX];
  ...
  private static int reduce_bit_usage(final Jlame_internal_flags gfc, final int gr, final int ch
      /* #if 0
      		, const float xr34orig[576], const float l3_xmin[SFBMAX], int maxbits
      #endif */
      ) {
    final JSessionConfig cfg = gfc.cfg;
    final Jgr_info cod_info = gfc.l3_side.tt[gr][ch];
    /*  try some better scalefac storage */
    Jtakehiro.best_scalefac_store(gfc, gr, ch, gfc.l3_side);
  ...
  static final int VBR_encode_frame(
      final Jlame_internal_flags gfc,
      final float xr34orig[][][] /*[2][2][576]*/,
      final float l3_xmin[][][] /*[2][2][SFBMAX]*/,
      final int max_bits[][] /*[2][2]*/) {
    final JSessionConfig cfg = gfc.cfg;
    final int sfwork_[][][] = new int[2][2][Jencoder.SFBMAX];
    final int vbrsfmin_[][][] = new int[2][2][Jencoder.SFBMAX];
    final Jalgo_s that_[][] = new Jalgo_s[2][2];
    ...
    int max_nbits_fr = 0;
    final int use_nbits_ch[][] = {
      {Jutil.MAX_BITS_PER_CHANNEL + 1, Jutil.MAX_BITS_PER_CHANNEL + 1},
      {Jutil.MAX_BITS_PER_CHANNEL + 1, Jutil.MAX_BITS_PER_CHANNEL + 1}
    };
    final int use_nbits_gr[] = {Jutil.MAX_BITS_PER_GRANULE + 1, Jutil.MAX_BITS_PER_GRANULE + 1};
    int use_nbits_fr = Jutil.MAX_BITS_PER_GRANULE + Jutil.MAX_BITS_PER_GRANULE;
    ...
    for (int gr = 0; gr < ngr; ++gr) {
      for (int ch = 0; ch < nch; ++ch) {
        if (max_bits[gr][ch] > 0) {
          final Jalgo_s that = that_[gr][ch];
    ...
    if (use_nbits_fr <= max_nbits_fr) {
      ok = true;
      for (int gr = 0; gr < ngr; ++gr) {
        if (use_nbits_gr[gr] > Jutil.MAX_BITS_PER_GRANULE) {
        ...
        for (int ch = 0; ch < nch; ++ch) {
          if (use_nbits_ch[gr][ch] > Jutil.MAX_BITS_PER_CHANNEL) {
     ...
     */
    {
      ...
      for (int gr = 0; gr < ngr; ++gr) {
        ...
        for (int ch = 0; ch < nch; ++ch) {
          if (use_nbits_ch[gr][ch] > Jutil.MAX_BITS_PER_CHANNEL) {
            max_nbits_ch[gr][ch] = Jutil.MAX_BITS_PER_CHANNEL;
          } else {
            max_nbits_ch[gr][ch] = use_nbits_ch[gr][ch];
          }
          max_nbits_gr[gr] += max_nbits_ch[gr][ch];
        }
        if (max_nbits_gr[gr] > Jutil.MAX_BITS_PER_GRANULE) {
          ...
          for (int ch = 0; ch < nch; ++ch) {
            if (max_nbits_ch[gr][ch] > 0) {
              f[ch] =
                  (float)
                      Math.sqrt(Math.sqrt((double) max_nbits_ch[gr][ch])); // TODO java: pow 0.25?
              s += f[ch];
            } else {
              f[ch] = 0;
            }
          ...
          for (int ch = 0; ch < nch; ++ch) {
            if (s > 0) {
              max_nbits_ch[gr][ch] = (int) (Jutil.MAX_BITS_PER_GRANULE * f[ch] / s);
            } else {
          ...
          if (nch > 1) {
            ...
            if (max_nbits_ch[gr][0] > Jutil.MAX_BITS_PER_CHANNEL) {
              max_nbits_ch[gr][0] = Jutil.MAX_BITS_PER_CHANNEL;
            }
            if (max_nbits_ch[gr][1] > Jutil.MAX_BITS_PER_CHANNEL) {
              max_nbits_ch[gr][1] = Jutil.MAX_BITS_PER_CHANNEL;
            }
          }
          max_nbits_gr[gr] = 0;
          for (int ch = 0; ch < nch; ++ch) {
      ...
      if (sum_fr > max_nbits_fr) {
        ...
        if (ngr > 1) {
          ...
          for (int gr = 0; gr < ngr; ++gr) {
            if (max_nbits_gr[gr] > Jutil.MAX_BITS_PER_GRANULE) {
              max_nbits_gr[gr] = Jutil.MAX_BITS_PER_GRANULE;
            }
          }
        }
        for (int gr = 0; gr < ngr; ++gr) {
          ...
          if (nch > 1) {
            ...
            for (int ch = 0; ch < nch; ++ch) {
              if (max_nbits_ch[gr][ch] > Jutil.MAX_BITS_PER_CHANNEL) {
                max_nbits_ch[gr][ch] = Jutil.MAX_BITS_PER_CHANNEL;
              }
            }
          }
      ...
      for (int gr = 0; gr < ngr; ++gr) {
        ...
        sum_fr += sum_gr;
        if (sum_gr > Jutil.MAX_BITS_PER_GRANULE) {
          ok = false;
        }
      }
      if (sum_fr > max_nbits_fr) {

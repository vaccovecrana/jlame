...
final class Jpsymodel {
  ...
  private static final float VO_SCALE = (1.f / (14752 * 14752) / (Jencoder.BLKSIZE / 2));
  ...
  */
  private static final float psycho_loudness_approx(final float[] energy, final float[] eql_w) {
    ...
    for (int i = 0; i < Jencoder.BLKSIZE / 2; ++i) {
  ...
  */
  ...
  // #define STATIC_ASSERT_EQUAL_DIMENSION(A,B) enum{static_assert_##A=1/((dimension_of(A) ==
  // dimension_of(B))?1:0)}
  ...
  private static final float vbrpsy_mask_add(float m1, float m2, final int b, final int delta) {
    ...
    if ((b >= 0 ? b : -b) <= delta) {
      ...
      if (ratio >= ma_max_i1) {
        ...
        final int i =
            (int)
                ((float) Math.log10((double) ratio)
                    * (16.0f)); // (Jutil.FAST_LOG10_X( ratio, 16.0f ));
   ...
   *
   * <p>partition band bo_s[sfb] is at the transition from scalefactor band sfb to the next one
   * sfb+1; enn and thmm have to be split between them
   */
  private static final void convert_partition2scalefac(
      final JPsyConst_CB2SB gd,
      final float[] eb,
      final float[] thr,
      final float enn_out[],
      final float thm_out[]) {
  ...
  private static final void convert_partition2scalefac_s(
    ...
    final JPsyConst_CB2SB gds = gfc.cd_psy.s;
    final float enn[] = new float[Jencoder.SBMAX_s];
    final float thm[] = new float[Jencoder.SBMAX_s];
    ...
    for (int sb = 0; sb < Jencoder.SBMAX_s; ++sb) {
  ...
  private static final void convert_partition2scalefac_l(
      final Jlame_internal_flags gfc, final float[] eb, final float[] thr, final int chn) {
    final JPsyStateVar psv = gfc.sv_psy;
    final JPsyConst_CB2SB gdl = gfc.cd_psy.l;
    final float[] enn = psv.en[chn].l; // [0];
    final float[] thm = psv.thm[chn].l; // [0];
    convert_partition2scalefac(gdl, eb, thr, enn, thm);
  }

  private static final void convert_partition2scalefac_l_to_s(
      final Jlame_internal_flags gfc, final float[] eb, final float[] thr, final int chn) {
    final JPsyStateVar psv = gfc.sv_psy;
    final JPsyConst_CB2SB gds = gfc.cd_psy.l_to_s;
    final float enn[] = new float[Jencoder.SBMAX_s];
    final float thm[] = new float[Jencoder.SBMAX_s];
    convert_partition2scalefac(gds, eb, thr, enn, thm);
    final float[][] en_chn_s = psv.en[chn].s; // java
  ...
  private static final float regcoef_s[] = {
    11.8f, /* these values are tuned only for 44.1kHz... */ 13.6f, 17.2f, 32f, 46.5f, 51.3f, 57.5f,
    67.1f, 71.5f, 84.6f, 97.6f, 130f,
    /*      255.8 */
  };

  private static final float pecalc_s(final JIII_psy_ratio mr, final float masking_lower) {
    float pe_s = 1236.28f / 4;
    final float[][] thm_s = mr.thm.s; // java
    final float[][] en_s = mr.en.s; // java
    for (int sb = 0; sb < Jencoder.SBMAX_s - 1; sb++) {
      ...
      for (int sblock = 0; sblock < 3; sblock++) {
        ...
        if (thm > 0.0f) {
          ...
          if (en > x) {
            if (en > x * 1e10f) {
              pe_s += regcoef_s[sb] * (10.0f * Jutil.LOG10);
            } else {
              pe_s +=
                  regcoef_s[sb]
                      * (float) Math.log10((double) (en / x)); // Jutil.FAST_LOG10( en / x );
            }
          }
        }
      }
  ...
  private static final float regcoef_l[] = {
    6.8f, /* these values are tuned only for 44.1kHz... */ 5.8f, 5.8f, 6.4f, 6.5f, 9.9f, 12.1f,
    14.4f, 15f, 18.9f, 21.6f, 26.9f, 34.2f, 40.2f, 46.8f, 56.5f, 60.7f, 73.9f, 85.7f, 93.4f, 126.1f,
    /*      241.3 */
  };

  private static final float pecalc_l(final JIII_psy_ratio mr, final float masking_lower) {
    final float[] thm_l = mr.thm.l; // java
    final float[] en_l = mr.en.l; // java
    float pe_l = 1124.23f / 4;
    for (int sb = 0; sb < Jencoder.SBMAX_l - 1; sb++) {
      ...
      if (thm > 0.0f) {
        ...
        if (en > x) {
          if (en > x * 1e10f) {
            pe_l += regcoef_l[sb] * (10.0f * Jutil.LOG10);
          } else {
            pe_l +=
                regcoef_l[sb]
                    * (float) Math.log10((double) (en / x)); // Jutil.FAST_LOG10( en / x );
  ...
  private static final void calc_energy(
      final JPsyConst_CB2SB l,
      final float[] fftenergy,
      final float[] eb,
      final float[] max,
      final float[] avg) {
  ...
  private static final void calc_mask_index_l(
      final Jlame_internal_flags gfc, final float[] max, final float[] avg, final byte[] mask_idx) {
    final JPsyConst_CB2SB gdl = gfc.cd_psy.l;
  ...
  private static final void vbrpsy_compute_fft_l(
      final Jlame_internal_flags gfc,
      final float buffer[ /*2*/][],
      final int boffset, // java
      final int chn,
      final int gr_out,
      final float fftenergy[ /*HBLKSIZE*/],
      final float wsamp_l[ /*BLKSIZE*/][],
      final int woffset) {
    final JPsyStateVar psv = gfc.sv_psy;

    final float[] wsamp_l_0 = wsamp_l[woffset]; // java
    if (chn < 2) {
      Jfft.fft_long(gfc, wsamp_l_0, chn, buffer, boffset);
    } else if (chn == 2) {
      final float sqrt2_half = Jutil.SQRT2 * 0.5f;
      /* FFT data for mid and side channel is derived from L & R */
      final float[] wsamp_l_1 = wsamp_l[woffset + 1];
      for (int j = Jencoder.BLKSIZE - 1; j >= 0; --j) {
    ...
    for (int j = Jencoder.BLKSIZE / 2 - 1; j >= 0; --j) {
      final int jm = Jencoder.BLKSIZE / 2 - j; // java
      final float re = wsamp_l_0[jm];
      final float im = wsamp_l_0[Jencoder.BLKSIZE / 2 + j];
      fftenergy[jm] = (re * re + im * im) * 0.5f;
    }
    /* total energy */
    {
      float totalenergy = 0.0f;
      for (int j = 11; j < Jencoder.HBLKSIZE; j++) {
  ...
  private static final void vbrpsy_compute_fft_s(
      final Jlame_internal_flags gfc,
      final float buffer[ /*2*/][],
      final int boffset, // java
      final int chn,
      final int sblock,
      final float fftenergy_s[ /*HBLKSIZE_s*/][],
      final float wsamp_s[ /*3*/][ /*BLKSIZE_s*/][],
      int woffset) {
    if (sblock == 0 && chn < 2) {
      Jfft.fft_short(gfc, wsamp_s[woffset], chn, buffer, boffset);
    }
    ...
    if (chn == 2) {
      ...
      final float sqrt2_half = Jutil.SQRT2 * 0.5f;
      /* FFT data for mid and side channel is derived from L & R */
      for (int j = Jencoder.BLKSIZE_s - 1; j >= 0; --j) {
        final float l = w0[j];
        final float r = w1[j];
        w0[j] = (l + r) * sqrt2_half;
        w1[j] = (l - r) * sqrt2_half;
    ...
    f[0] = w0[0];
    f[0] *= f[0];
    for (int j = Jencoder.BLKSIZE_s / 2 - 1; j >= 0; --j) {
      final int jm = Jencoder.BLKSIZE_s / 2 - j; // java
      final float re = w0[jm];
      final float im = w0[Jencoder.BLKSIZE_s / 2 + j];
   ...
   *********************************************************************/
  private static final void vbrpsy_compute_loudness_approximation_l(
      final Jlame_internal_flags gfc,
      final int gr_out,
      final int chn,
      final float fftenergy[ /*HBLKSIZE*/]) {
   ...
   **********************************************************************/
  private static final void vbrpsy_attack_detection(
      final Jlame_internal_flags gfc,
      final float buffer[ /*2*/][],
      int boffset,
      final int gr_out,
      final JIII_psy_ratio masking_ratio[ /*2*/][ /*2*/],
      final JIII_psy_ratio masking_MS_ratio[ /*2*/][ /*2*/],
      final float energy[ /*4*/],
      final float sub_short_factor[ /*4*/][ /*3*/],
      final int ns_attacks[ /*4*/][ /*4*/],
    ...
    final int n_chn_psy = (cfg.mode == Jlame.JOINT_STEREO) ? 4 : n_chn_out;

    /* Don't copy the input buffer into a temporary buffer */
    /* unroll the loop 2 times */
    boffset += 576 - 350 - NSFIRLEN + 192;
    final JIII_psy_ratio[] masking_ratio_gr_out = masking_ratio[gr_out]; // java
    ...
    for (int chn = 0; chn < n_chn_psy; chn++) {
      ...
      for (int i = 0; i < 3; ++i) {
        final float enn = en_subshort[i * 3 + 3] + en_subshort[i * 3 + 4] + en_subshort[i * 3 + 5];
        float factor = 1.f;
  ...
  private static final void vbrpsy_skip_masking_s(
      final Jlame_internal_flags gfc, final int chn, final int sblock) {
    if (sblock == 0) {
  ...
  private static final void vbrpsy_calc_mask_index_s(
      final Jlame_internal_flags gfc, final float[] max, final float[] avg, final byte[] mask_idx) {
    final JPsyConst_CB2SB gds = gfc.cd_psy.s;
    final int last_tab_entry = tab.length - 1;
    int b = 0;
  ...
  private static final void vbrpsy_compute_masking_s(
      final Jlame_internal_flags gfc,
      final float fftenergy_s[ /*HBLKSIZE_s*/][],
      final float[] eb,
      final float[] thr,
    ...
    final JPsyStateVar psv = gfc.sv_psy;
    final JPsyConst_CB2SB gds = gfc.cd_psy.s;
    final float max[] = new float[Jencoder.CBANDS]; // java: already zeroed
    final float avg[] = new float[Jencoder.CBANDS]; // java: already zeroed
    final byte mask_idx_s[] = new byte[Jencoder.CBANDS]; // tab.length max
    ...
    for (; b < Jencoder.CBANDS; ++b) {
      eb[b] = 0;
      thr[b] = 0;
    }
  }

  private static final void vbrpsy_compute_masking_l(
      final Jlame_internal_flags gfc,
      final float fftenergy[ /*HBLKSIZE*/],
      final float eb_l[ /*CBANDS*/],
      final float thr[ /*CBANDS*/],
      final int chn) {
    final JPsyStateVar psv = gfc.sv_psy;
    final JPsyConst_CB2SB gdl = gfc.cd_psy.l;
    final float max[] = new float[Jencoder.CBANDS];
    final float avg[] = new float[Jencoder.CBANDS];
    final byte mask_idx_l[] = new byte[Jencoder.CBANDS + 2];
    ...
    for (; b < gdl.npart; b++) {
      ...
      */
      if (blocktype_old == Jencoder.SHORT_TYPE) {
        ...
        if (blocktype_old == Jencoder.NORM_TYPE) {
  ...
  private static final void vbrpsy_compute_block_type(
      final JSessionConfig cfg, final boolean[] uselongblock) {
    final int short_blocks = cfg.short_blocks; // java
    if (short_blocks == Jlame_global_flags.short_block_coupled
    ...
    final boolean is_short_block_dispensed =
        (short_blocks == Jlame_global_flags.short_block_dispensed);
    final boolean is_short_block_forced = (short_blocks == Jlame_global_flags.short_block_forced);
  ...
  private static final void vbrpsy_apply_block_type(
      final JPsyStateVar psv,
      final int nch,
      final boolean[] uselongblock,
      final int[] blocktype_d) {
    final int[] blocktype_old = psv.blocktype_old; // java
    /* update the blocktype of the previous granule, since it depends on what
     * happend in this granule */
    for (int chn = 0; chn < nch; chn++) {
      int blocktype = Jencoder.NORM_TYPE;
      /* disable short blocks */

      int blocktype_old_chn = blocktype_old[chn]; // java
      if (uselongblock[chn]) {
        /* no attack : use long blocks */
        if (blocktype_old_chn == Jencoder.SHORT_TYPE) {
          blocktype = Jencoder.STOP_TYPE;
        }
      } else {
        /* attack : use short blocks */
        blocktype = Jencoder.SHORT_TYPE;
        if (blocktype_old_chn == Jencoder.NORM_TYPE) {
          blocktype_old_chn = Jencoder.START_TYPE;
        }
        if (blocktype_old_chn == Jencoder.STOP_TYPE) {
          blocktype_old_chn = Jencoder.SHORT_TYPE;
        }
      }
   ...
   ***************************************************************/
  private static final void vbrpsy_compute_MS_thresholds(
      final float eb[ /*4*/][ /*CBANDS*/],
      final float thr[ /*4*/][ /*CBANDS*/],
      final float cb_mld[ /*CBANDS*/],
      final float ath_cb[ /*CBANDS*/],
      final float athlower,
      final float msfix,
      final int n) {
  ...
  /**
   * NOTE: the bitrate reduction from the inter-channel masking effect is low compared to the chance
   * of getting annyoing artefacts. L3psycho_anal_vbr does not use this feature. (Robert 071216)
   */
  static final int L3psycho_anal_vbr(
      final Jlame_internal_flags gfc,
      final float buffer[ /*2*/][],
      final int boffset,
      final int gr_out,
      final JIII_psy_ratio masking_ratio[][], // [2][2],
      final JIII_psy_ratio masking_MS_ratio[][], // [2][2],
      final float percep_entropy[ /*2*/],
      final float percep_MS_entropy[ /*2*/],
      final float energy[ /*4*/],
      final int blocktype_d[ /*2*/]) {
    final JSessionConfig cfg = gfc.cfg;
    final JPsyStateVar psv = gfc.sv_psy;
    final JPsyConst_CB2SB gdl = gfc.cd_psy.l;
    final JPsyConst_CB2SB gds = gfc.cd_psy.s;
    ...
    final float fftenergy[] = new float[Jencoder.HBLKSIZE];
    final float fftenergy_s[][] = new float[3][Jencoder.HBLKSIZE_s];
    final float wsamp_L[][] = new float[2][Jencoder.BLKSIZE];
    final float wsamp_S[][][] = new float[2][3][Jencoder.BLKSIZE_s];
    final float eb[][] = new float[4][Jencoder.CBANDS];
    final float thr[][] = new float[4][Jencoder.CBANDS];

    final float sub_short_factor[][] = new float[4][3];
    final float pcfact = 0.6f;
    final float ath_factor =
    ...
    /* chn=2 and 3 = Mid and Side channels */
    final int n_chn_psy = (cfg.mode == Jlame.JOINT_STEREO) ? 4 : cfg.channels_out;

    final JIII_psy_xmin last_thm[] = new JIII_psy_xmin[4];
    last_thm[0] = new JIII_psy_xmin(psv.thm[0]);
    last_thm[1] = new JIII_psy_xmin(psv.thm[1]);
    last_thm[2] = new JIII_psy_xmin(psv.thm[2]);
    last_thm[3] = new JIII_psy_xmin(psv.thm[3]);
    ...
    /* LONG BLOCK CASE */
    {
      ...
      if (cfg.mode == Jlame.JOINT_STEREO) {
        if (uselongblock[0] & uselongblock[1]) {
          vbrpsy_compute_MS_thresholds(
              eb, thr, gdl.mld_cb, gfc.ATH.cb_l, ath_factor, cfg.msfix, gdl.npart);
        }
      }
    ...
    /* SHORT BLOCKS CASE */
    {
      ...
      for (int sblock = 0; sblock < 3; sblock++) {
        ...
        if (cfg.mode == Jlame.JOINT_STEREO) {
          if (!(uselongblock[0] | uselongblock[1])) {
            vbrpsy_compute_MS_thresholds(
                eb, thr, gds.mld_cb, gfc.ATH.cb_s, ath_factor, cfg.msfix, gds.npart);
          }
        }
        /* TODO: apply adaptive ATH masking here ?? */
        for (int chn = 0; chn < n_chn_psy; ++chn) {
     ...
     *********************************************************************/
    final boolean blocktype =
        (blocktype_d[0] == Jencoder.SHORT_TYPE || blocktype_d[1] == Jencoder.SHORT_TYPE); // java
    for (int chn = 0; chn < n_chn_psy; chn++) {
      float[] appe; // java
      int ppe, type;
      JIII_psy_ratio mr;
      ...
      if (chn > 1) {
        ...
        ppe = chn - 2;
        type = Jencoder.NORM_TYPE;
        if (blocktype) {
          type = Jencoder.SHORT_TYPE;
        }
  ...
  private static final float s3_func(final float bark) {
    ...
    tempx += 0.474;
    final float tempy =
        15.811389f + 7.5f * tempx - 17.5f * (float) Math.sqrt((double) (1.0f + tempx * tempx));
  ...
  private static final void init_numline(
      ...
      final int sbmax,
      final int[] scalepos) {
    final float b_frq[] = new float[Jencoder.CBANDS + 1];
    final float mdct_freq_frac = sfreq / (2.0f * mdct_size);
    final float deltafreq = fft_size / (2.0f * mdct_size);
    final int partition[] = new int[Jencoder.HBLKSIZE]; // = { 0 };
    sfreq /= fft_size;
    ...
    int i = 0;
    for (; i < Jencoder.CBANDS; i++) {
      final float bark1 = Jutil.freq2bark(sfreq * j);

      b_frq[i] = sfreq * j;

      int j2;
      for (j2 = j; Jutil.freq2bark(sfreq * j2) - bark1 < DELBARK && j2 <= fftsize2; j2++) {
        ;
      }
  ...
  private static final void compute_bark_values(
      final JPsyConst_CB2SB gd,
      float sfreq,
      final int fft_size,
      final float[] bval,
      final float[] bval_width) {
    ...
    for (int k = 0, j = 0; k < ni; k++) {
      ...
      float bark1 = Jutil.freq2bark(sfreq * (float) j);
      float bark2 = Jutil.freq2bark(sfreq * (float) (w - 1));
      bval[k] = .5f * (bark1 + bark2);

      bark1 = Jutil.freq2bark(sfreq * ((float) j - .5f));
      bark2 = Jutil.freq2bark(sfreq * ((float) w - .5f));
  ...
  /** @return java: p array */
  private static final float[] init_s3_values(
      /*final float[][] p,*/ final int s3ind[ /*2*/][],
      final int npart,
      final float[] bval,
      final float[] bval_width,
      final float[] norm) {
    final float s3[][] = new float[Jencoder.CBANDS][Jencoder.CBANDS]; // java: already zeroed
  ...
  static final int psymodel_init(final Jlame_global_flags gfp) {
    ...
    final float bval[] = new float[Jencoder.CBANDS];
    final float bval_width[] = new float[Jencoder.CBANDS];
    final float norm[] = new float[Jencoder.CBANDS]; // java: already zeroed
    ...
    final JPsyConst gd = new JPsyConst();
    gfc.cd_psy = gd;

    gd.force_short_block_calc = gfp.experimentalZ;

    psv.blocktype_old[0] =
        psv.blocktype_old[1] = Jencoder.NORM_TYPE; /* the vbr header is long blocks */

    for (int i = 0; i < 4; ++i) {
      for (int j = 0; j < Jencoder.CBANDS; ++j) {
        psv.nb_l1[i][j] = 1e20f;
        psv.nb_l2[i][j] = 1e20f;
        psv.nb_s1[i][j] = psv.nb_s2[i][j] = 1.0f;
      }
      for (int sb = 0; sb < Jencoder.SBMAX_l; sb++) {
        psv.en[i].l[sb] = 1e20f;
    ...
    init_numline(gd.l, sfreq, Jencoder.BLKSIZE, 576, Jencoder.SBMAX_l, gfc.scalefac_band.l);
    compute_bark_values(gd.l, sfreq, Jencoder.BLKSIZE, bval, bval_width);
    ...
    for (int i = 0; i < gd.l.npart; i++) {
      ...
      for (int k = 0; k < gd.l.numlines[i]; k++, j++) {
        final float freq = sfreq * j / (1000.0f * Jencoder.BLKSIZE);
        float level;
        /* freq = Min(.1,freq); */
        /* ATH below 100 Hz constant, not further climbing */
        level =
            Jutil.ATHformula(cfg, freq * 1000)
     ...
     ************************************************************************/
    init_numline(gd.s, sfreq, Jencoder.BLKSIZE_s, 192, Jencoder.SBMAX_s, gfc.scalefac_band.s);
    compute_bark_values(gd.s, sfreq, Jencoder.BLKSIZE_s, bval, bval_width);
    ...
    for (int i = 0; i < gd.s.npart; i++) {
      ...
      for (int k = 0, numlines_i = gd.s.numlines[i]; k < numlines_i; k++, j++) {
        final float freq = sfreq * j / (1000.0f * Jencoder.BLKSIZE_s);
        float level;
        /* freq = Min(.1,freq); */
        /* ATH below 100 Hz constant, not further climbing */
        level =
    ...
    gd.decay = (float) Math.exp(-1.0 * Jutil.LOG10 / (temporalmask_sustain_sec * sfreq / 192.0));
    ...
    if (cfg.ATHtype != -1) {
      ...
      final float freq_inc = (float) cfg.samplerate / (float) (Jencoder.BLKSIZE);
    ...
    init_numline(gd.l_to_s, sfreq, Jencoder.BLKSIZE, 192, Jencoder.SBMAX_s, gfc.scalefac_band.s);
    return 0;
  }
}

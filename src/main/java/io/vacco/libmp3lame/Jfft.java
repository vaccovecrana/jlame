...
/**
 * * FFT and FHT routines * Copyright 1988, 1993; Ron Mayer * Copyright (c) 1999-2000 Takehiro
 * Tominaga * * fht(fz,n); * Does a hartley transform of "n" points in the array "fz". * * NOTE:
 * This routine uses at least 2 patented algorithms, and may be * under the restrictions of a bunch
 * of different organizations. * Although I wrote it completely myself; it is kind of a derivative *
 * of a routine I once authored and released under the GPL, so it * may fall under the free software
 * foundation's restrictions; * it was worked on as a Stanford Univ project, so they claim * some
 * rights to it; it was further optimized at work here, so * I think this company claims parts of
 * it. The patents are * held by R. Bracewell (the FHT algorithm) and O. Buneman (the * trig
 * generator), both at Stanford Univ. * If it were up to me, I'd say go do whatever you want with
 * it; * but it would be polite to give credit to the following people * if you use this anywhere: *
 * Euler - probable inventor of the fourier transform. * Gauss - probable inventor of the FFT. *
 * Hartley - probable inventor of the hartley transform. * Buneman - for a really cool trig
 * generator * Mayer(me) - for authoring this particular version and * including all the
 * optimizations in one package. * Thanks, * Ron Mayer; mayer@acuson.com * and added some
 * optimization by * Mather - idea of using lookup table * Takehiro - some dirty hack for speed up
 */
...
final class Jfft {
  ...
  private static final void fht(final float[] fz, final int foffset, int n) {
    ...
    do {
      ...
      do {
        ...
        f1 = fz[gi + 0] - fz[gi + k1];
        f0 = fz[gi + 0] + fz[gi + k1];
        f3 = Jutil.SQRT2 * fz[gi + k3];
        f2 = Jutil.SQRT2 * fz[gi + k2];
        fz[gi + k2] = f0 - f2;
        fz[gi + 0] = f0 + f2;
        fz[gi + k3] = f1 - f3;
        fz[gi + k1] = f1 + f3;
  ...
  */
  static final void fft_short(
      final Jlame_internal_flags gfc,
      final float x_real[ /*3*/][ /*BLKSIZE_s*/],
      final int chn,
    ...
    do {
      ...
      int xi = Jencoder.BLKSIZE_s / 2; // x[xi]
      final int k = (576 / 3) * (b + 1) + boffset;
      int j = Jencoder.BLKSIZE_s / 8 - 1;
      do {
        ...
        x[Jencoder.BLKSIZE_s / 2 + xi++] = f0 + f2;
        x[Jencoder.BLKSIZE_s / 2 + xi++] = f1 + f3;
        x[Jencoder.BLKSIZE_s / 2 + xi++] = f0 - f2;
        x[Jencoder.BLKSIZE_s / 2 + xi] = f1 - f3;
        xi -= 3;
      } while (--j >= 0);

      // gfc.fft_fht( x, Jencoder.BLKSIZE_s / 2 );
      fht(x, xi, Jencoder.BLKSIZE_s / 2);
  ...
  */
  static final void fft_long(
      final Jlame_internal_flags gfc,
      final float x[ /*BLKSIZE*/],
      final int chn,
      final float buffer[ /*2*/][],
      final int boffset) {
    final float[] buf = buffer[chn]; // java
    int jj = Jencoder.BLKSIZE / 8 - 1;
    int xoffset = Jencoder.BLKSIZE / 2;
    ...
    do {
      ...
      x[Jencoder.BLKSIZE / 2 + xoffset++] = f0 + f2;
      x[Jencoder.BLKSIZE / 2 + xoffset++] = f1 + f3;
      x[Jencoder.BLKSIZE / 2 + xoffset++] = f0 - f2;
      x[Jencoder.BLKSIZE / 2 + xoffset] = f1 - f3;
      xoffset -= 3;
    } while (--jj >= 0);

    // gfc.fft_fht( x, Jencoder.BLKSIZE / 2 );
    fht(x, xoffset, Jencoder.BLKSIZE / 2);
    /* BLKSIZE/2 because of 3DNow! ASM routine */
  }

  static final void init_fft(final Jlame_internal_flags gfc) {
    /* The type of window used here will make no real difference, but */
    /* in the interest of merging nspsytune stuff - switch to blackman window */
    for (int i = 0; i < Jencoder.BLKSIZE; i++) {
      /* blackman window */
      gfc.cd_psy.window[i] =
          (float)
              (0.42
                  - 0.5 * Math.cos(2.0 * Math.PI * (i + .5) / Jencoder.BLKSIZE)
                  + 0.08 * Math.cos(4. * Math.PI * (i + .5) / Jencoder.BLKSIZE));
    }

    for (int i = 0; i < Jencoder.BLKSIZE_s / 2; i++) {
      gfc.cd_psy.window_s[i] =
          (float) (0.5 * (1.0 - Math.cos(2.0 * Math.PI * (i + 0.5) / Jencoder.BLKSIZE_s)));
    }
  }
}

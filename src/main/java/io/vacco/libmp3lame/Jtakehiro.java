...
final class Jtakehiro {

  static final int LARGE_BITS = 100000;

  private static final class Jsubdv_table {
    private final int region0_count;
    private final int region1_count;

    Jsubdv_table(final int r0, final int r1) {
      this.region0_count = r0;
      this.region1_count = r1;
    }
  }

  private static final Jsubdv_table subdv_table[] = { // [23] = {
    new Jsubdv_table(0, 0), /* 0 bands */
    new Jsubdv_table(0, 0), /* 1 bands */
    new Jsubdv_table(0, 0), /* 2 bands */
    new Jsubdv_table(0, 0), /* 3 bands */
    new Jsubdv_table(0, 0), /* 4 bands */
    new Jsubdv_table(0, 1), /* 5 bands */
    new Jsubdv_table(1, 1), /* 6 bands */
    new Jsubdv_table(1, 1), /* 7 bands */
    new Jsubdv_table(1, 2), /* 8 bands */
    new Jsubdv_table(2, 2), /* 9 bands */
    new Jsubdv_table(2, 3), /* 10 bands */
    new Jsubdv_table(2, 3), /* 11 bands */
    new Jsubdv_table(3, 4), /* 12 bands */
    new Jsubdv_table(3, 4), /* 13 bands */
    new Jsubdv_table(3, 4), /* 14 bands */
    new Jsubdv_table(4, 5), /* 15 bands */
    new Jsubdv_table(4, 5), /* 16 bands */
    new Jsubdv_table(4, 6), /* 17 bands */
    new Jsubdv_table(5, 6), /* 18 bands */
    new Jsubdv_table(5, 6), /* 19 bands */
    new Jsubdv_table(5, 7), /* 20 bands */
    new Jsubdv_table(6, 7), /* 21 bands */
    new Jsubdv_table(6, 7) /* 22 bands */
  };
   ...
   *********************************************************************/
  private static final void quantize_lines_xrpow_01(
      int l, final float istep, final float[] xr, int xroffset, final int[] ix, int ixoffset) {
  ...
  private static final void quantize_lines_xrpow(
      int l, final float istep, final float[] xp, int xoffset, final int[] pi, int pioffset) {
    /* if( Jquantize_pvt.TAKEHIRO_IEEE754_HACK ) {
    		...
    		while( l-- != 0 ) {
    			...
    			fi0 = x0 + Jquantize_pvt.adj43[ Float.floatToIntBits( fi0 ) - MAGIC_INT ];
    			fi1 = x1 + Jquantize_pvt.adj43[ Float.floatToIntBits( fi1 ) - MAGIC_INT ];
    			fi2 = x2 + Jquantize_pvt.adj43[ Float.floatToIntBits( fi2 ) - MAGIC_INT ];
    			fi3 = x3 + Jquantize_pvt.adj43[ Float.floatToIntBits( fi3 ) - MAGIC_INT ];
    ...
    while (l-- != 0) {
      ...
      x0 += Jquantize_pvt.adj43[rx0]; // QUANTFAC(rx0);
      rx3 = (int) x3; // XRPOW_FTOI(x3, rx3);
      x1 += Jquantize_pvt.adj43[rx1]; // QUANTFAC(rx1);
      pi[pioffset++] = (int) x0; // XRPOW_FTOI(x0, *ix++);
      x2 += Jquantize_pvt.adj43[rx2]; // QUANTFAC(rx2);
      pi[pioffset++] = (int) x1; // XRPOW_FTOI(x1, *ix++);
      x3 += Jquantize_pvt.adj43[rx3]; // QUANTFAC(rx3);
  ...
  */
  ...
  private static final void quantize_xrpow(
      final float[] xp,
      final int[] pi,
      final float istep,
      final Jgr_info cod_info,
      final Jcalc_noise_data prev_noise) {
    ...
    final boolean prev_data_use =
        (prev_noise != null && (cod_info.global_gain == prev_noise.global_gain));

    final int sfbmax = (cod_info.block_type == Jencoder.SHORT_TYPE) ? 38 : 21;

    for (int sfb = 0; sfb <= sfbmax; sfb++) {
      int step = -1;

      if (prev_data_use || cod_info.block_type == Jencoder.NORM_TYPE) {
        step =
            cod_info.global_gain
                - ((cod_info.scalefac[sfb] + (cod_info.preflag ? Jquantize_pvt.pretab[sfb] : 0))
  ...
  private static final long count_bit_ESC(
      final int[] ix, int ixoffset, final int end, int t1, final int t2, int s) {
    /* ESC-table is used */
    final int linbits = (Jtables.ht[t1].xlen << 16) + Jtables.ht[t2].xlen;
    int sum = 0, sum2;
  ...
  private static final long count_bit_noESC(
      final int[] ix, int ixoffset, final int end, final int mx, int s) {
    /* No ESC-words */
    int sum1 = 0;
    final byte[] hlen1 = Jtables.ht[1].hlen;
  ...
  private static final long count_bit_noESC_from2(
      final int[] ix, int ixoffset, final int end, final int max, int s) {
    int t1 = huf_tbl_noESC[max - 1];
    /* No ESC-words */
    final int xlen = Jtables.ht[t1].xlen;
    final int[] table = (t1 == 2) ? Jtables.table23 : Jtables.table56;
    int sum = 0, sum2;
  ...
  private static final long count_bit_noESC_from3(
      final int[] ix, int ixoffset, final int end, final int max, int s) {
    ...
    int sum3 = 0;
    final int xlen = Jtables.ht[t1].xlen;
    final byte[] hlen1 = Jtables.ht[t1].hlen;
    final byte[] hlen2 = Jtables.ht[t1 + 1].hlen;
    final byte[] hlen3 = Jtables.ht[t1 + 2].hlen;
  ...
  */
  ...
  private static final long choose_table_nonMMX(
      final int[] ix, final int start, final int end, final int s) {
    ...
    if (max > Jquantize_pvt.IXMAX_VAL) {
      // s[0] = LARGE_BITS;
      // return -1;
    ...
    for (; choice2 < 32; choice2++) {
      if (Jtables.ht[choice2].linmax >= max) {
        break;
      }
    }
    int choice = choice2 - 8;
    for (; choice < 24; choice++) {
      if (Jtables.ht[choice].linmax >= max) {
        break;
      }
    }
  ...
  /*	      count_bit							 */
  /*************************************************************************/
  static final int noquant_count_bits(
      final Jlame_internal_flags gfc, final Jgr_info gi, final Jcalc_noise_data prev_noise) {
    ...
    if (gi.block_type == Jencoder.SHORT_TYPE) {
    ...
    } else if (gi.block_type == Jencoder.NORM_TYPE) {
      ...
      gi.region1_count = Jencoder.SBMAX_l - 1 - 7 - 1;
  ...
  static final int count_bits(
    ...
    final float w = (Jquantize_pvt.IXMAX_VAL) / Jquantize_pvt.ipow20[gi.global_gain];

    if (gi.xrpow_max > w) {
      return LARGE_BITS;
    }

    quantize_xrpow(xr, ix, Jquantize_pvt.ipow20[gi.global_gain], gi, prev_noise);

    if ((gfc.sv_qnt.substep_shaping & 2) != 0) {
      int j = 0;
      /* 0.634521682242439 = 0.5946*2**(.5*0.1875) */
      final int gain = gi.global_gain + gi.scalefac_scale;
      final float roundfac = 0.634521682242439f / Jquantize_pvt.ipow20[gain];
   ...
   **********************************************************************/
  private static final void recalc_divide_init(
      final Jlame_internal_flags gfc,
      final Jgr_info cod_info,
      final int[] ix,
      final int r01_bits[],
  ...
  private static final void recalc_divide_sub(
      final Jlame_internal_flags gfc,
      final Jgr_info cod_info2,
      final Jgr_info gi,
      final int[] ix,
      final int r01_bits[],
      final int r01_div[],
    ...
    for (int r2 = 2, r = 0; r2 < Jencoder.SBMAX_l + 1; r2++, r++) {
  ...
  static final void best_huffman_divide(final Jlame_internal_flags gfc, final Jgr_info gi) {
    final JSessionConfig cfg = gfc.cfg;
    final int[] ix = gi.l3_enc;
    ...
    if (gi.block_type == Jencoder.SHORT_TYPE && cfg.mode_gr == 1) {
      return;
    }

    final Jgr_info cod_info2 = new Jgr_info(gi);
    if (gi.block_type == Jencoder.NORM_TYPE) {
    ...
    if (cod_info2.block_type == Jencoder.NORM_TYPE) {
      recalc_divide_sub(gfc, cod_info2, gi, ix, r01_bits, r01_div, r0_tbl, r1_tbl);
    } else {
      /* Count the number of bits necessary to code the bigvalues region. */
  ...
  private static final void scfsi_calc(final int ch, final JIII_side_info l3_side) {
    final Jgr_info gi = l3_side.tt[1][ch];
    final int[] scalefac0 = l3_side.tt[0][ch].scalefac;
    final int[] scfsi_ch = l3_side.scfsi[ch]; // java
    final int[] scalefac = gi.scalefac; // java

    int sfb;
    for (int i = 0; i < Jtables.scfsi_band.length - 1; i++) {
      final int be = Jtables.scfsi_band[i + 1]; // java
      for (sfb = Jtables.scfsi_band[i]; sfb < be; sfb++) {
   ...
   */
  static final void best_scalefac_store(
      final Jlame_internal_flags gfc, final int gr, final int ch, final JIII_side_info l3_side) {
    final JSessionConfig cfg = gfc.cfg;
    final Jgr_info[][] tt = l3_side.tt; // java
    /* use scalefac_scale if we can */
    final Jgr_info gi = tt[gr][ch];
    ...
    if (!gi.preflag && gi.block_type != Jencoder.SHORT_TYPE && cfg.mode_gr == 2) {
      int sfb;
      for (sfb = 11; sfb < Jencoder.SBPSY_l; sfb++) {
        if (scalefac[sfb] < Jquantize_pvt.pretab[sfb] && scalefac[sfb] != -2) {
          break;
        }
      }
    ...
    if (cfg.mode_gr == 2
        && gr == 1
        && tt[0][ch].block_type != Jencoder.SHORT_TYPE
        && tt[1][ch].block_type != Jencoder.SHORT_TYPE) {
  ...
  private static final boolean mpeg1_scale_bitcount(
  ...
  /**
   * Since no bands have been over-amplified, we can set scalefac_compress and slen[] for the
   * formatter
   */
  ...
  private static final boolean mpeg2_scale_bitcount(
      final Jlame_internal_flags gfc, final Jgr_info cod_info) {
  ...
  static final boolean scale_bitcount(final Jlame_internal_flags gfc, final Jgr_info cod_info) {
  ...
  static final void huffman_init(final Jlame_internal_flags gfc) {

...
import io.vacco.libmp3lame.JEncStateVar.Jheader;
...
final class Jbitstream {
  ...
  /**
   * unsigned int is at least this large: we work with ints, so when doing bit manipulation, we
   * limit ourselves to MAX_LENGTH-2 just to be on the safe side
   */
  // private static final int MAX_LENGTH = 32;

  private static final int calcFrameLength(
      final JSessionConfig cfg, final int kbps, final int pad) {
  ...
  static final int get_max_frame_buffer_size_by_constraint(
      final JSessionConfig cfg, final int constraint) {
    int maxmp3buf = 0;
  ...
  private static final void putbits_noheaders(
      final Jlame_internal_flags gfc, final int val, int j) {
    final Jbitstream bs = gfc.bs;
    final byte[] buf = bs.buf; // java
    int buf_bit_idx = bs.buf_bit_idx; // java
  ...
  /*
    ...
    the ancillary data...
  */
  private static final void drain_into_ancillary(
      final Jlame_internal_flags gfc, int remainingBits) {
    final JSessionConfig cfg = gfc.cfg;
    final JEncStateVar esv = gfc.sv_enc;
  ...
  private static final void encodeSideInfo2(
      final Jlame_internal_flags gfc, final int bitsPerFrame) {
    final JSessionConfig cfg = gfc.cfg;
    final JEncResult eov = gfc.ov_enc;
    final JEncStateVar esv = gfc.sv_enc;
  ...
  private static final int Huffmancode(
  ...
  */
  ...
  /**
   * compute the number of bits required to flush all mp3 frames currently in the buffer. This
   * should be the same as the reservoir size. Only call this routine between frames - i.e. only
   * after all headers and data have been added to the buffer by format_bitstream().
   *
   * <p>Also compute total_bits_output = size of mp3 buffer (including frame headers which may not
   * have yet been send to the mp3 buffer) + number of bits needed to flush all mp3 frames.
   *
   * <p>total_bytes_output is the size of the mp3 output buffer if lame_encode_flush_nogap() was
   * called right now.
   *
   * @return (flushbits) | (total_bytes_output << 32)
   */
  static final long compute_flushbits(
      final Jlame_internal_flags gfc /*, final int[] total_bytes_output*/) {
    final JSessionConfig cfg = gfc.cfg;
    final JEncStateVar esv = gfc.sv_enc;
   ...
   * format_bitstream()
   *
   * <p>This is called after a frame of audio has been quantized and coded. It will write the
   * encoded audio to the bitstream. Note that from a layer3 encoder's perspective the bit stream is
   * primarily a series of main_data() blocks, with header and side information inserted at the
   * proper locations to maintain framing. (See Figure A.7 in the IS).
   */
  ...
  static final int do_copy_buffer(
      final Jlame_internal_flags gfc, final byte[] buffer, final int offset, final int size) {
    final Jbitstream bs = gfc.bs;
   ...
   *
   * <p>mp3data=0 indicates data in buffer is an id3tags and VBR tags mp3data=1 data is real mp3
   * frame data.
   */
  static final int copy_buffer(
      final Jlame_internal_flags gfc,
      final byte[] buffer,
      final int offset,
      final int size,
      final boolean mp3data) {
    ...
    if (minimum > 0 && mp3data) {
      ...
      /**
       * sum number of bytes belonging to the mp3 stream this info will be written into the
       * Xing/LAME header for seeking
       */

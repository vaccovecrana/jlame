...
public final class JVbrTag {
  ...
  /**
   * 4 bytes for Header Tag 4 bytes for Header Flags 100 bytes for entry (NUMTOCENTRIES) 4 bytes for
   * FRAME SIZE 4 bytes for STREAM_SIZE 4 bytes for VBR SCALE. a VBR quality indicator: 0=best
   * 100=worst 20 bytes for LAME tag. for example, "LAME3.12 (beta 6)" ___________ 140 bytes
   */
  private static final int VBRHEADERSIZE = (NUMTOCENTRIES + 4 + 4 + 4 + 4 + 4);

  private static final int LAMEHEADERSIZE =
  ...
  /**
   * Lookup table for fast CRC computation See 'CRC_update_lookup' Uses the polynomial
   * x^16+x^15+x^2+1
   */
   ...
   ***********************************************************************/
  private static final void addVbr(final JVBR_seek_info v, final int bitrate) {
    v.nVbrNumFrames++;
    v.sum += bitrate;
    v.seen++;
  ...
  private static final void Xing_seek_table(final JVBR_seek_info v, final byte[] t) {
    if (v.pos <= 0) {
      return;
    }
  ...
  */
   ...
   *                              (in Bytes NOT Bits)
   ****************************************************************************/
  static final void AddVbrFrame(final Jlame_internal_flags gfc) {
    final int kbps = Jtables.bitrate_table[gfc.cfg.version][gfc.ov_enc.bitrate_index];
  ...
  private static final boolean IsVbrTag(final byte[] buf, final int offset) {
    ...
    final boolean isTag0 =
        ((buf[offset] == VBRTag0[0])
            && (buf[offset + 1] == VBRTag0[1])
            && (buf[offset + 2] == VBRTag0[2])
            && (buf[offset + 3] == VBRTag0[3]));
    final boolean isTag1 =
        ((buf[offset] == VBRTag1[0])
            && (buf[offset + 1] == VBRTag1[1])
            && (buf[offset + 2] == VBRTag1[2])
            && (buf[offset + 3] == VBRTag1[3]));

    return (isTag0 || isTag1);
  }

  private static final void SHIFT_IN_BITS_VALUE(
      final byte[] x, final int off, final int n, final int v) {
    x[off] = (byte) ((((int) x[off] & 0xff) << n) | (v & ~(-1 << n)));
  }

  private static final void setLameTagFrameHeader(
      final Jlame_internal_flags gfc, final byte[] buffer) {
    final JSessionConfig cfg = gfc.cfg;
    final JEncResult eov = gfc.ov_enc;

    SHIFT_IN_BITS_VALUE(
        buffer, 0, 8, 0xff); // ( buffer[0] = (buffer[0] << 8) | ( 0xff & ~(-1 << 255) ) )
    ...
    SHIFT_IN_BITS_VALUE(
        ...
        1,
        1,
        cfg.version); // ( buffer[1] = (buffer[1] << 1) | ( (cfg.version) & ~(-1 << 1) ) )
    SHIFT_IN_BITS_VALUE(
        buffer, 1, 2, 4 - 3); // ( buffer[1] = (buffer[1] << 2) | ( (4 - 3) & ~(-1 << 2) ) )
    SHIFT_IN_BITS_VALUE(
        buffer,
        1,
        1,
    ...
    SHIFT_IN_BITS_VALUE(
        ...
        2,
        4,
        eov.bitrate_index); // ( buffer[2] = (buffer[2] << 4) | ( eov.bitrate_index & ~(-1 << 4) ) )
    SHIFT_IN_BITS_VALUE(
        buffer,
        2,
        2,
    ...
    SHIFT_IN_BITS_VALUE(
        ...
        2,
        1,
        cfg.extension ? 1 : 0); // ( buffer[2] = (buffer[2] << 1) | ( cfg.extension & ~(-1 << 1) ) )

    SHIFT_IN_BITS_VALUE(
        buffer, 3, 2, cfg.mode); // ( buffer[3] = (buffer[3] << 2) | ( cfg.mode & ~(-1 << 2) ) )
    SHIFT_IN_BITS_VALUE(
        buffer,
        3,
        2,
        eov.mode_ext); // ( buffer[3] = (buffer[3] << 2) | ( eov.mode_ext & ~(-1 << 2) ) )
    SHIFT_IN_BITS_VALUE(
        buffer,
        3,
        1,
        cfg.copyright ? 1 : 0); // ( buffer[3] = (buffer[3] << 1) | ( cfg.copyright & ~(-1 << 1) ) )
    SHIFT_IN_BITS_VALUE(
        buffer,
        3,
        1,
        cfg.original ? 1 : 0); // ( buffer[3] = (buffer[3] << 1) | ( cfg.original & ~(-1 << 1) ) )
    SHIFT_IN_BITS_VALUE(
        buffer,
        3,
        2,
        cfg.emphasis); // ( buffer[3] = (buffer[3] << 2) | ( cfg.emphasis & ~(-1 << 2) ) )
    ...
    int bbyte; // byte
    {
      ...
      if (cfg.free_format) {
        ...
        bbyte = (byte) (Jutil.BitrateIndex(bitrate, cfg.version, cfg.samplerate) << 4);
      }
    }
  ...
  public static final boolean GetVbrTag(final JVBRTAGDATA pTagData, final byte[] buf) {
    int offset = 0;
    /* get Vbr header data */
    pTagData.flags = 0;
    ...
    h_bitrate = Jtables.bitrate_table[h_id][h_bitrate];

    /* check for FFE syncword */
    if ((((int) buf[1] & 0xff) >> 4) == 0xE) {
      pTagData.samprate = Jtables.samplerate_table[2][h_sr_index];
    } else {
      pTagData.samprate = Jtables.samplerate_table[h_id][h_sr_index];
    }
    /* if( h_id == 0 ) */
    /*  pTagData.samprate >>= 1; */
   ...
   *****************************************************************************/
  static final int InitVbrTag(final Jlame_global_flags gfp) {
    ...
    /* write dummy VBR tag of all 0's into bitstream */
    {
      ...
      for (int i = 0; i < n; ++i) {
        Jbitstream.add_dummy_byte(gfc, buffer[i], 1);
      }
    }
    /* Success */
    return 0;
   ...
   *****************************************************************************/
  private static final int PutLameVBR(
      final Jlame_global_flags gfp,
      final int nMusicLength,
      final byte[] pbtStreamBuffer,
      int nBytesWritten /*offset*/,
      char crc) {
    final Jlame_internal_flags gfc = gfp.internal_flags;
    final JSessionConfig cfg = gfc.cfg;
    ...
    */
    final String szVersion = Jlame_version.get_lame_tag_encoder_short_version();
    final int nRevision = 0x00;
    final byte vbr_type_translator[] = {
      1, 5, 3, 2, 4, 0, 3
    }; /*numbering different in vbr_mode vs. Lame tag */

    final byte nLowpass =
        (byte) (((cfg.lowpassfreq / 100.0f) + .5f) > 255 ? 255 : (cfg.lowpassfreq / 100.0f) + .5f);
    ...
    switch (cfg.mode) {
      ...
      case Jlame.STEREO:
        nStereoMode = 1;
        break;
      case Jlame.DUAL_CHANNEL:
        nStereoMode = 2;
        break;
      case Jlame.JOINT_STEREO:
        if (cfg.force_ms) {
          nStereoMode = 4;
        } else {
          nStereoMode = 3;
    ...
    if (cfg.short_blocks == Jlame_global_flags.short_block_forced
        || cfg.short_blocks == Jlame_global_flags.short_block_dispensed
        || ((cfg.lowpassfreq == -1) && (cfg.highpassfreq == -1))
        || /* "-k" */ (cfg.disable_reservoir && cfg.avg_bitrate < 320)
        || cfg.noATH
        || cfg.ATHonly
        || (nAthType == 0)
    ...
    final byte nMisc =
        (byte) (nNoiseShaping + (nStereoMode << 2) + (bNonOptimal << 5) + (nSourceFreq << 6));
    ...
    pbtStreamBuffer[nBytesWritten] =
        (byte) (enc_delay >> 4); /* works for win32, does it for unix? */
  ...
  public static final int lame_get_lametag_frame(
    ...
    final byte btToc[] =
        new byte[NUMTOCENTRIES]; // java: already zeroed /* Clear all TOC entries */
    ...
    final int stream_size =
        gfc.VBR_seek_table.nBytesWritten
            + gfc.VBR_seek_table.TotalFrameSize; // TODO java check uint32
    ...
    if (cfg.error_protection) {
      ...
      Jbitstream.CRC_writeheader(gfc, buffer);
    }
    {
   ...
   *****************************************************************************/
  static final int PutVbrTag(final Jlame_global_flags gfp, final RandomAccessFile fpStream) {
    final Jlame_internal_flags gfc = gfp.internal_flags;

...
public final class Jlame {

  private static final int LAME_DEFAULT_QUALITY = 3;

  /**
   * maximum size of albumart image (128KB), which affects LAME_MAXMP3BUFFER as well since
   * lame_encode_buffer() also returns ID3v2 tag data
   */
  private static final int LAME_MAXALBUMART = (128 * 1024);

  /**
   * maximum size of mp3buffer needed if you encode at most 1152 samples for each call to
   * lame_encode_buffer. see lame_encode_buffer() below (LAME_MAXMP3BUFFER is now obsolete)
   */
  ...
  private static final void lame_init_params_ppflt(final Jlame_internal_flags gfc) {
  ...
  private static final class Jband_pass {
    // private final int bitrate;     /* only indicative value */
    private final int lowpass;
    //
    private Jband_pass(/*final int br,*/ final int lp) {
      // bitrate = br;
      lowpass = lp;
    }
  }

  private static final Jband_pass freq_map[] = {
    new Jband_pass(2000), // new Jband_pass( 8, 2000 ),
    new Jband_pass(3700), // new Jband_pass( 16, 3700 ),
    new Jband_pass(3900), // new Jband_pass( 24, 3900 ),
    new Jband_pass(5500), // new Jband_pass( 32, 5500 ),
    new Jband_pass(7000), // new Jband_pass( 40, 7000 ),
    new Jband_pass(7500), // new Jband_pass( 48, 7500 ),
    new Jband_pass(10000), // new Jband_pass( 56, 10000 ),
    new Jband_pass(11000), // new Jband_pass( 64, 11000 ),
    new Jband_pass(13500), // new Jband_pass( 80, 13500 ),
    new Jband_pass(15100), // new Jband_pass( 96, 15100 ),
    new Jband_pass(15600), // new Jband_pass( 112, 15600 ),
    new Jband_pass(17000), // new Jband_pass( 128, 17000 ),
    new Jband_pass(17500), // new Jband_pass( 160, 17500 ),
    new Jband_pass(18600), // new Jband_pass( 192, 18600 ),
    new Jband_pass(19400), // new Jband_pass( 224, 19400 ),
    new Jband_pass(19700), // new Jband_pass( 256, 19700 ),
    new Jband_pass(20500) // new Jband_pass( 320, 20500 )
  };
  /**
   * @param lowerlimit Output. best lowpass frequency limit for input filter in Hz
   * @param upperlimit Output. best highpass frequency limit for input filter in Hz
   * @param bitrate Input. total bitrate in kbps
   * @return java: lowerlimit, best lowpass frequency limit for input filter in Hz
   */
  private static final double optimum_bandwidth(
      /*final double[] lowerlimit, final double[] upperlimit,*/ final int bitrate) {
    final int table_index = Jutil.nearestBitrateFullIndex(bitrate);
  ...
  private static final void lame_init_qval(final Jlame_global_flags gfp) {
    final Jlame_internal_flags gfc = gfp.internal_flags;
    final JSessionConfig cfg = gfc.cfg;
  ...
  private static final class Jq_map {
    private final int sr_a;
    final float qa, qb, ta, tb; // final int lp;// FIXME lp never uses
    //
    private Jq_map(
        final int sr,
        final float a,
  ...
  private static final Jq_map m[] = { // [9]
    new Jq_map(48000, 0.0f, 6.5f, 0.0f, 6.5f), // , 23700 ),
    new Jq_map(44100, 0.0f, 6.5f, 0.0f, 6.5f), // , 21780 ),
    new Jq_map(32000, 6.5f, 8.0f, 5.2f, 6.5f), // , 15800 ),
    new Jq_map(24000, 8.0f, 8.5f, 5.2f, 6.0f), // , 11850 ),
    new Jq_map(22050, 8.5f, 9.01f, 5.2f, 6.5f), // , 10892 ),
    new Jq_map(16000, 9.01f, 9.4f, 4.9f, 6.5f), // ,  7903 ),
    new Jq_map(12000, 9.4f, 9.6f, 4.5f, 6.0f), // ,  5928 ),
    new Jq_map(11025, 9.6f, 9.9f, 5.1f, 6.5f), // ,  5446 ),
    new Jq_map(8000, 9.9f, 10.f, 4.9f, 6.5f), // ,  3952 )
  };
  /********************************************************************
   ...
   *
   ********************************************************************/
  public static final int lame_init_params(final Jlame_global_flags gfp) {
    ...
    gfc.class_id = Jlame_internal_flags.LAME_ID;
    gfc.lame_init_params_successful =
        false; /* will be set to one, when we get through until the end */

    if (gfp.num_channels < 1 || 2 < gfp.num_channels) {
      return -1;
    } /* number of input channels makes no sense */
    if (gfp.samplerate != 0) {
      if (Jutil.SmpFrqIndex(gfp.samplerate) < 0) {
        return -1;
    ...
    if (cfg.vbr == vbr_off && gfp.brate == 0) {
      ...
      if (Jquantize.EQ(gfp.compression_ratio, 0)) {
    ...
    if (gfp.VBR == vbr_off && gfp.compression_ratio > 0) {
       ...
       */
      gfp.brate =
          (int) (gfp.samplerate * (cfg.channels_out << 4) / (1.e3f * gfp.compression_ratio));

      /* we need the version for the bitrate table look up */
      cfg.samplerate_index = Jutil.SmpFrqIndex(gfp.samplerate);
      cfg.version = Jutil.getVersion(gfp.samplerate);
      // assert( cfg->samplerate_index >= 0 );

      if (!cfg.free_format) {
        gfp.brate = Jutil.FindNearestBitrate(gfp.brate, cfg.version, gfp.samplerate);
      }
    }
     ...
     *******************************************************/
    cfg.samplerate_index = Jutil.SmpFrqIndex(cfg.samplerate);
    cfg.version = Jutil.getVersion(cfg.samplerate);
    // assert( cfg.samplerate_index >= 0 );
    ...
    if (cfg.vbr == vbr_off) {
      if (cfg.free_format) {
        ...
        gfp.brate = Jutil.FindNearestBitrate(gfp.brate, cfg.version, cfg.samplerate);
        gfc.ov_enc.bitrate_index = Jutil.BitrateIndex(gfp.brate, cfg.version, cfg.samplerate);
    ...
    Jbitstream.init_bit_stream_w(gfc);

    final int j = cfg.samplerate_index + (3 * cfg.version) + (cfg.samplerate < 16000 ? 6 : 0);
    for (int i = 0; i < Jencoder.SBMAX_l + 1; i++) {
      gfc.scalefac_band.l[i] = Jquantize_pvt.sfBandIndex[j].l[i];
    }

    for (int i = 0; i < Jencoder.PSFB21 + 1; i++) {
      final int size = (gfc.scalefac_band.l[22] - gfc.scalefac_band.l[21]) / Jencoder.PSFB21;
      final int start = gfc.scalefac_band.l[21] + i * size;
      gfc.scalefac_band.psfb21[i] = start;
    }
    gfc.scalefac_band.psfb21[Jencoder.PSFB21] = 576;

    for (int i = 0; i < Jencoder.SBMAX_s + 1; i++) {
      gfc.scalefac_band.s[i] = Jquantize_pvt.sfBandIndex[j].s[i];
    }

    for (int i = 0; i < Jencoder.PSFB12 + 1; i++) {
      final int size = (gfc.scalefac_band.s[13] - gfc.scalefac_band.s[12]) / Jencoder.PSFB12;
      final int start = gfc.scalefac_band.s[12] + i * size;
      gfc.scalefac_band.psfb12[i] = start;
    }
    gfc.scalefac_band.psfb12[Jencoder.PSFB12] = 192;
      ...
      case vbr_mtrh:
        {
          ...
          Jpresets.apply_preset(gfp, 500 - (gfp.VBR_q * 10), false);
      ...
      default: /* cbr/abr */
        {
          ...
          Jpresets.apply_preset(gfp, gfp.VBR_mean_bitrate_kbps, false);
          gfp.VBR = cfg.vbr;
    ...
    if (cfg.vbr != vbr_off) {
      ...
      if (gfp.VBR_min_bitrate_kbps != 0) {
        gfp.VBR_min_bitrate_kbps =
            Jutil.FindNearestBitrate(gfp.VBR_min_bitrate_kbps, cfg.version, cfg.samplerate);
        cfg.vbr_min_bitrate_index =
            Jutil.BitrateIndex(gfp.VBR_min_bitrate_kbps, cfg.version, cfg.samplerate);
      ...
      if (gfp.VBR_max_bitrate_kbps != 0) {
        gfp.VBR_max_bitrate_kbps =
            Jutil.FindNearestBitrate(gfp.VBR_max_bitrate_kbps, cfg.version, cfg.samplerate);
        cfg.vbr_max_bitrate_index =
            Jutil.BitrateIndex(gfp.VBR_max_bitrate_kbps, cfg.version, cfg.samplerate);
      ...
      gfp.VBR_min_bitrate_kbps = Jtables.bitrate_table[cfg.version][cfg.vbr_min_bitrate_index];
      gfp.VBR_max_bitrate_kbps = Jtables.bitrate_table[cfg.version][cfg.vbr_max_bitrate_index];
      gfp.VBR_mean_bitrate_kbps =
          Math.min(
              Jtables.bitrate_table[cfg.version][cfg.vbr_max_bitrate_index],
              gfp.VBR_mean_bitrate_kbps);
      gfp.VBR_mean_bitrate_kbps =
          Math.max(
              Jtables.bitrate_table[cfg.version][cfg.vbr_min_bitrate_index],
              gfp.VBR_mean_bitrate_kbps);
    }
    ...
    if (gfp.short_blocks == Jlame_global_flags.short_block_not_set) {
      gfp.short_blocks = Jlame_global_flags.short_block_allowed;
    }
    ...
    if (gfp.short_blocks == Jlame_global_flags.short_block_allowed
        && (cfg.mode == JOINT_STEREO || cfg.mode == STEREO)) {
      gfp.short_blocks = Jlame_global_flags.short_block_coupled;
    }
    ...
    if (cfg.vbr == vbr_off) {
      gfc.sv_enc.slot_lag =
          gfc.sv_enc.frac_SpF =
              (int) (((cfg.version + 1) * 72000L * cfg.avg_bitrate) % cfg.samplerate);
    }

    lame_init_bitstream(gfp);

    Jquantize_pvt.iteration_init(gfc);
    Jpsymodel.psymodel_init(gfp);

    cfg.buffer_constraint = Jbitstream.get_max_frame_buffer_size_by_constraint(cfg, gfp.strict_ISO);
  ...
  private static final int update_inbuffer_size(
      final Jlame_internal_flags gfc, final int nsamples) {
  ...
  private static final int calcNeeded(final JSessionConfig cfg) {
    ;
    int pcm_samples_per_frame = 576 * cfg.mode_gr;

    int mf_needed =
        Jencoder.BLKSIZE + pcm_samples_per_frame - Jencoder.FFTOFFSET; /* amount needed for FFT */
  ...
  /**
   * THE MAIN LAME ENCODING INTERFACE mt 3/00
   *
   * <p>input pcm data, output (maybe) mp3 frames. This routine handles all buffering, resampling
   * and filtering for you. The required mp3buffer_size can be computed from num_samples, samplerate
   * and encoding rate, but here is a worst case estimate:
   *
   * <p>mp3buffer_size in bytes = 1.25*num_samples + 7200
   *
   * <p>return code = number of bytes output in mp3buffer. can be 0
   *
   * <p>NOTE: this routine uses LAME's internal PCM data representation, 'sample_t'. It should not
   * be used by any application. applications should use lame_encode_buffer(),
   * lame_encode_buffer_float() lame_encode_buffer_int() etc... depending on what type of data they
   * are working with.
   */
  private static final int lame_encode_buffer_sample_t(
      final Jlame_internal_flags gfc,
      int nsamples,
      final byte[] mp3buf,
      int mp3buf_offset,
      final int mp3buf_size) {
    if (gfc.class_id != Jlame_internal_flags.LAME_ID) {
      return -3;
    }
    ...
    int mp3size = Jbitstream.copy_buffer(gfc, mp3buf, mp3buf_offset, buf_size, false);
    // }
    if (mp3size < 0) {
      return mp3size;
    ...
    while (nsamples > 0) {
      ...
      final long tmp =
          Jutil.fill_buffer(gfc, mfbuf, in_buffer, in_buffer_ptr, nsamples /*, &n_in, &n_out*/);
      ...
      if (esv.mf_samples_to_encode < 1) {
        esv.mf_samples_to_encode = Jencoder.ENCDELAY + Jencoder.POSTDELAY;
      }
      ...
      if (esv.mf_size >= mf_needed) {
        ...
        final int ret =
            Jencoder.lame_encode_mp3_frame(
                gfc, mfbuf[0], mfbuf[1], mp3buf, mp3buf_offset, buf_size);
        ...
        for (int ch = 0, channels_out = cfg.channels_out, mf_size = esv.mf_size;
            ch < channels_out;
            ch++) {
  ...
  private static final void lame_copy_inbuffer(
      final Jlame_internal_flags gfc,
      final Object l,
      final Object r,
      final int offset,
      final int nsamples,
      final int /* PCMSampleType */ pcm_type,
      final int jump,
      final float s) {
    final JSessionConfig cfg = gfc.cfg;
    final JEncStateVar esv = gfc.sv_enc;
  ...
  private static final int lame_encode_buffer_template(
      final Jlame_global_flags gfp,
      final Object buffer_l,
      final Object buffer_r,
      final int offset,
      final int nsamples,
      final byte[] mp3buf,
      final int mp3buf_offset,
      final int mp3buf_size,
      final int /* PCMSampleType */ pcm_type,
      final int aa,
      final float norm) {
  ...
  /**
   * input pcm data, output (maybe) mp3 frames. This routine handles all buffering, resampling and
   * filtering for you.
   *
   * <p>return code number of bytes output in mp3buf. Can be 0 -1: mp3buf was too small -2: malloc()
   * problem -3: lame_init_params() not called -4: psycho acoustic problems
   *
   * <p>The required mp3buf_size can be computed from num_samples, samplerate and encoding rate, but
   * here is a worst case estimate:
   *
   * <p>mp3buf_size in bytes = 1.25*num_samples + 7200
   *
   * <p>I think a tighter bound could be: (mt, March 2000) MPEG1: num_samples*(bitrate/8)/samplerate
   * + 4*1152*(bitrate/8)/samplerate + 512 MPEG2: num_samples*(bitrate/8)/samplerate +
   * 4*576*(bitrate/8)/samplerate + 256
   *
   * <p>but test first if you use that!
   *
  ...
  /**
   * as lame_encode_buffer, but for 'float's. !! NOTE: !! data must still be scaled to be in the
   * same range as short int, +/- 32768
   */
  public static final int lame_encode_buffer_float(
      final Jlame_global_flags gfp,
      final float pcm_l[],
      final float pcm_r[],
      final int nsamples,
  ...
  /**
   * as lame_encode_buffer, but for 'float's. !! NOTE: !! data must be scaled to +/- 1 full scale
   */
  public static final int lame_encode_buffer_ieee_float(
      final Jlame_global_flags gfp,
      final float pcm_l[],
      final float pcm_r[],
      final int nsamples,
  ...
  public static final int lame_encode_buffer_interleaved_ieee_float(
      final Jlame_global_flags gfp,
      final float pcm[],
      final int nsamples,
      final byte[] mp3buf,
  ...
  /**
   * as lame_encode_buffer, but for 'double's. !! NOTE: !! data must be scaled to +/- 1 full scale
   */
  public static final int lame_encode_buffer_ieee_double(
      final Jlame_global_flags gfp,
      final double pcm_l[],
      final double pcm_r[],
      final int nsamples,
  ...
  public static final int lame_encode_buffer_interleaved_ieee_double(
      final Jlame_global_flags gfp,
      final double pcm[],
      final int nsamples,
      final byte[] mp3buf,
  ...
  public static final int lame_encode_buffer_int(
      final Jlame_global_flags gfp,
      final int pcm_l[],
      final int pcm_r[],
      final int offset,
  ...
  /**
   * Same as lame_encode_buffer_long(), but with correct scaling. !! NOTE: !! data must still be
   * scaled to be in the same range as type 'long'. Data should be in the range: +/-
   * 2^(8*size(long)-1)
   */
  public static final int lame_encode_buffer_long2(
      final Jlame_global_flags gfp,
      final long pcm_l[],
      final long pcm_r[],
      final int nsamples,
    ...
    return lame_encode_buffer_template(
        gfp, pcm_l, pcm_r, 0, nsamples, mp3buf, mp3buf_offset, mp3buf_size, pcm_long_type, 1, norm);
  }

  /**
   * as lame_encode_buffer, but for long's !! NOTE: !! data must still be scaled to be in the same
   * range as short int, +/- 32768
   *
   * <p>This scaling was a mistake (doesn't allow one to exploit full precision of type 'long'. Use
   * lame_encode_buffer_long2() instead.
   */
  public static final int lame_encode_buffer_long(
      final Jlame_global_flags gfp,
      final long pcm_l[],
      final long pcm_r[],
      final int nsamples,
    ...
    /* input is assumed to be normalized to +/- 32768 for full scale */
    return lame_encode_buffer_template(
        gfp, pcm_l, pcm_r, 0, nsamples, mp3buf, mp3buf_offset, mp3buf_size, pcm_long_type, 1, 1.0f);
  }
  ...
  public static final int lame_encode_buffer_interleaved(
    ...
    return lame_encode_buffer_template(
        gfp, pcm, null, 0, nsamples, mp3buf, mp3buf_offset, mp3buf_size, pcm_short_type, 2, 1.0f);
  }

  public static final int lame_encode_buffer_interleaved_int(
      final Jlame_global_flags gfp,
      final int pcm[],
      final int nsamples,
      final byte[] mp3buf,
    ...
    return lame_encode_buffer_template(
        gfp, pcm, null, 0, nsamples, mp3buf, mp3buf_offset, mp3buf_size, pcm_int_type, 2, norm);
  }
   ...
   * with no errors.
   *********************************************************************/
  public static final int lame_encode_flush_nogap(
      final Jlame_global_flags gfp, final byte[] mp3buffer, int mp3buffer_size) {
    int rc = -3;
    if (gfp.is_lame_global_flags_valid()) {
      final Jlame_internal_flags gfc = gfp.internal_flags;
      if (gfc.is_lame_internal_flags_valid()) {
        Jbitstream.flush_bitstream(gfc);
        /* if user specifed buffer size = 0, dont check size */
        if (mp3buffer_size == 0) {
          mp3buffer_size = Integer.MAX_VALUE;
        }
        rc = Jbitstream.copy_buffer(gfc, mp3buffer, 0, mp3buffer_size, true);
      }
    }
    return rc;
  }
  ...
  /**
   * called by lame_init_params. You can also call this after flush_nogap if you want to write new
   * id3v2 and Xing VBR tags into the bitstream
   */
  public static final int lame_init_bitstream(final Jlame_global_flags gfp) {
    if (gfp.is_lame_global_flags_valid()) {
      final Jlame_internal_flags gfc = gfp.internal_flags;
      if (gfc != null) {
  ...
  /*****************************************************************/
  public static final int lame_encode_flush(
      final Jlame_global_flags gfp, final byte[] mp3buffer, final int mp3buffer_size) {
    if (!gfp.is_lame_global_flags_valid()) {
      return -3;
    }
    final Jlame_internal_flags gfc = gfp.internal_flags;
    ...
    final int samples_to_encode = esv.mf_samples_to_encode - Jencoder.POSTDELAY;
    ...
    while (frames_left > 0 && imp3 >= 0) {
      ...
      imp3 =
          lame_encode_buffer(
              gfp, buffer[0], buffer[1], bunch, mp3buffer, offset, mp3buffer_size_remaining);
    ...
    Jbitstream.flush_bitstream(gfc);
    imp3 = Jbitstream.copy_buffer(gfc, mp3buffer, offset, mp3buffer_size_remaining, true);
    if (imp3 < 0) {
      /* some type of fatal error */
      return imp3;
    }
    ...
    if (gfp.write_id3tag_automatic) {
      ...
      Jid3tag.id3tag_write_v1(gfp);

      imp3 = Jbitstream.copy_buffer(gfc, mp3buffer, offset, mp3buffer_size_remaining, false);
   ...
   *
   ***********************************************************************/
  public static final int lame_close(final Jlame_global_flags gfp) {
    int ret = 0;
    if (gfp != null && gfp.class_id == Jlame_internal_flags.LAME_ID) {
      final Jlame_internal_flags gfc = gfp.internal_flags;
      gfp.class_id = 0;
      if (null == gfc || gfc.class_id != Jlame_internal_flags.LAME_ID) {
        ret = -3;
      }
      if (null != gfc) {
   ...
   */
  /*private static final int lame_encode_finish(final Jlame_global_flags gfp, final byte[] mp3buffer, final int mp3buffer_size)
  {
  ...
  /*****************************************************************/
  /**
   * OPTIONAL: lame_mp3_tags_fid will rewrite a Xing VBR tag to the mp3 file with file pointer fid.
   * These calls perform forward and backwards seeks, so make sure fid is a real file. Make sure
   * lame_encode_flush has been called, and all mp3 data has been written to the file before calling
   * this function. NOTE: if VBR tags are turned off by the user, or turned off by LAME because the
   * output is not a regular file, this call does nothing NOTE: LAME wants to read from the file to
   * skip an optional ID3v2 tag, so make sure you opened the file for writing and reading. NOTE: You
   * can call lame_get_lametag_frame instead, if you want to insert the lametag yourself.
   */
  public static final void lame_mp3_tags_fid(
      final Jlame_global_flags gfp, final RandomAccessFile fpStream) {
    if (!gfp.is_lame_global_flags_valid()) {
      return;
    }
    final Jlame_internal_flags gfc = gfp.internal_flags;
    ...
    try {
      if (fpStream != null) {
        fpStream.seek(0);
        final int rc = JVbrTag.PutVbrTag(gfp, fpStream);
        switch (rc) {
          default:
            /* OK */
            break;
  ...
  private static final int lame_init_internal_flags(final Jlame_internal_flags gfc) {
    if (null == gfc) {
      return -1;
    }
     ...
     */
    gfc.sv_enc.mf_samples_to_encode = Jencoder.ENCDELAY + Jencoder.POSTDELAY;
    gfc.sv_enc.mf_size =
        Jencoder.ENCDELAY - Jencoder.MDCTDELAY; /* we pad input with this many 0's */
    gfc.ov_enc.encoder_padding = 0;
    gfc.ov_enc.encoder_delay = Jencoder.ENCDELAY;
  ...
  private static final int lame_init_old(final Jlame_global_flags gfp) {
    // gfp.clear();// java: don't do it, because init is being called only after creating of the gfp

    gfp.class_id = Jlame_internal_flags.LAME_ID;
    ...
    gfp.num_channels = 2;
    gfp.num_samples = Jutil.MAX_U_32_NUM;

    gfp.write_lame_tag = true;
    gfp.quality = -1;
    gfp.short_blocks = Jlame_global_flags.short_block_not_set;
    gfp.subblock_gain = -1;
    ...
    gfp.internal_flags = new Jlame_internal_flags();
  ...
  public static final Jlame_global_flags lame_init() {
    Jutil.init_log_table();

    final Jlame_global_flags gfp = new Jlame_global_flags();

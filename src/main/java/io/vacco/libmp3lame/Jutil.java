...
public final class Jutil {
  ...
  private static final float ATHformula_GB(
      float f, final float value, final float f_min, final float f_max) {
  ...
  static final float freq2bark(float freq) {
    ...
    freq *= 0.001f;
    return 13.0f * (float) Math.atan((double) (.76f * freq))
        + 3.5f * (float) Math.atan((double) (freq * freq / (7.5f * 7.5f)));
  }
  ...
  /**
   * Used to find table index when we need bitrate-based values determined using tables
   *
   * <p>bitrate in kbps
   *
   * <p>Gabriel Bouvigne 2002-11-03
   */
  ...
  /**
   * copy in new samples from in_buffer into mfbuf, with resampling if necessary. n_in = number of
   * samples from the input buffer that were used. n_out = number of samples copied into mfbuf
   *
   * @return java: n_out | (n_in << 32)
   */
  static final long fill_buffer(
      final Jlame_internal_flags gfc,
      final float mfbuf[ /* 2 */][],
      final float in_buffer[ /* 2 */][],
      final int inoffset,

...
final class Jgr_info {
  final float xr[] = new float[576];
  final int l3_enc[] = new int[576];
  final int scalefac[] = new int[Jencoder.SFBMAX];
  float xrpow_max;
  ...
  int psymax;
  int sfbdivide;
  final int width[] = new int[Jencoder.SFBMAX];
  final int window[] = new int[Jencoder.SFBMAX];
  int count1bits;
  /* added for LSF */
  ...
  final boolean energy_above_cutoff[] = new boolean[Jencoder.SFBMAX];
  //
  Jgr_info() {}

  Jgr_info(final Jgr_info g) {
    copyFrom(g);
  }

  final void copyFrom(final Jgr_info g) {
    System.arraycopy(g.xr, 0, this.xr, 0, 576);
    System.arraycopy(g.l3_enc, 0, this.l3_enc, 0, 576);
    System.arraycopy(g.scalefac, 0, this.scalefac, 0, Jencoder.SFBMAX);
    ...
    System.arraycopy(g.width, 0, this.width, 0, Jencoder.SFBMAX);
    System.arraycopy(g.window, 0, this.window, 0, Jencoder.SFBMAX);
    ...
    System.arraycopy(g.energy_above_cutoff, 0, this.energy_above_cutoff, 0, Jencoder.SFBMAX);
  }
   ...
   *
   ************************************************************************/
  final float init_xrpow_core_c(
      final float xrpow[ /*576*/], final int upper /*, final float[] sum*/) {
    float sum = 0;

 ...
 * </pre>
 *
 * Calling ExitMP3() do not closing the input stream. The input stream must be closed manually;
 */
public final class Jmpg123 extends Jmpstr_tag {
   ...
   */
  public Jmpg123(final int sampleSize, final boolean isSigned, final boolean isBigEndian)
  ...
  private final boolean isSyncwordMp123(final byte[] data) {
    ...
    if ((data[1] & 0x18) == 0x18
        && (data[1] & 0x06) == 0x04
        && ((sAbl2[((int) data[2] & 0xff) >> 4] & (1 << (((int) data[3] & 0xff) >> 6))) != 0)) {
      return false;
    }
    if ((data[3] & 3) == 2) {
   ...
   * number of mono samples output. either 576 or 1152 depending on MP3 file.
   */
  private final int decodeHeaders(
      final byte[] buffer, final int len, final float[] p, final int psize) {

...
import io.vacco.libmp3lame.JVBRTAGDATA;
import io.vacco.libmp3lame.JVbrTag;
import java.util.Arrays;

/** java: wordbuf[ wordpointer ] */
public class Jmpstr_tag {
  private static final class Jbuf {
    private byte[] pnt;
    private int size;
    private int pos;
    private Jbuf next;
  ...
  //
  private static final Isynth sDecoder = new Decode(); // short[] version;
  private static final Isynth sDecoderUnclipped = new DecodeUnclipped(); // float[] version;
  //
  ...
  public final Jframe fr = new Jframe();

  final JIII_sideinfo sideinfo = new JIII_sideinfo();
  /** bit stream space used ???? */
  /* MAXFRAMESIZE */
  final byte bsspace[][] = new byte[2][Jmpg123.MAXFRAMESIZE + 1024];

  final float hybrid_block[][][] = new float[2][2][Jmpg123.SBLIMIT * Jmpg123.SSLIMIT];
  ...
  */
  ...
  public final void InitMP3() {
    Jlayer1.hip_init_tables_layer1();
    Jlayer2.hip_init_tables_layer2();
    Jlayer3.hip_init_tables_layer3();
    ...
    // }
    Jdct64.make_decode_tables(32767);
  }
  ...
  public final void ExitMP3() {
    ...
    while (b != null) {
      b.pnt = null;
      final Jbuf bn = b.next;
      b = bn;
    }
    // }
  }

  private final Jbuf addbuf(final byte[] buf, final int size) {
    final Jbuf nbuf = new Jbuf();
  ...
  */
  private final int check_vbr_header(final int bytes) {
    ...
    final JVBRTAGDATA pTagData = new JVBRTAGDATA();
    ...
    this.vbr_header = JVbrTag.GetVbrTag(pTagData, xing);
    if (this.vbr_header) {
      this.num_frames = pTagData.frames;
      this.enc_delay = pTagData.enc_delay;
      this.enc_padding = pTagData.enc_padding;

      /* Jutil.lame_report_fnc(this.report_msg,"hip: delays: %d %d \n",this.enc_delay,this.enc_padding); */
      /* Jutil.lame_report_fnc(this.report_msg,"hip: Xing VBR header dectected.  MP3 file has %d frames\n", pTagData.frames); */
    ...
    for (int i = 0; i < this.bsize; i++) {
      ...
      if (i >= 3) {
        ...
        if (h && free_match) {
          ...
          final int stereo = (mode == Jmpg123.MPG_MD_MONO) ? 1 : 2;
  ...
  private final int decodeMP3_clipchoice(
      final byte[] in, final int isize, final Object out, final int[] done, final Isynth synth) {
    if (in != null && isize != 0 && addbuf(in, isize) == null) {
      return Jmpg123.MP3_ERR;
    }
    ...
    if (!this.header_parsed) {
      ...
      if (this.fsizeold == -1 || this.sync_bitstream) {
        ...
        if (this.vbr_header) {
          ...
          if (vbrbytes > this.bsize) {
            /* Jutil.lame_report_fnc(this.report_err,"hip: not enough data to parse entire Xing header\n"); */
            return Jmpg123.MP3_NEED_MORE;
          }
      ...
      if (bytes < 0) {
        /* Jutil.lame_report_fnc(this.report_err,"hip: need more bytes %d\n", bytes); */
        return Jmpg123.MP3_NEED_MORE;
      }
      if (bytes > 0) {
        ...
        if (size > Jmpg123.MAXFRAMESIZE) {
          /* wordpointer buffer is trashed.  probably cant recover, but try anyway */
          System.err.printf(
              "hip: wordpointer trashed.  size=%d (%d)  bytes=%d \n",
              size, Jmpg123.MAXFRAMESIZE, bytes);
          size = 0;
          this.wordbuf = this.bsspace[this.bsnum];
          this.wordpointer = 512;
        }
        ...
        int i = (size + bytes) - Jmpg123.MAXFRAMESIZE;
        for (; i > 0; --i) {
          --bytes;
          read_buf_byte();
        }
    ...
    if (!this.side_parsed) {
      ...
      if (this.fr.lay == 3) {
        ...
        int bits = Jlayer3.decode_layer3_sideinfo(this);
    ...
    int iret = Jmpg123.MP3_NEED_MORE;
    if (!this.data_parsed) {
      ...
      switch (this.fr.lay) {
        case 1:
          ...
          // Jlayer1.decode_layer1_frame( this, out, done );// FIXME incorrect calling
          if (Jlayer1.decode_layer1_frame(this, out, done, synth) < 0) {
            return Jmpg123.MP3_ERR;
          }
          break;
        ...
        case 2:
          ...
          // Jlayer2.decode_layer2_frame( this, out, done );// FIXME incorrect calling
          Jlayer2.decode_layer2_frame(this, out, done, synth);
          break;

        case 3:
          Jlayer3.decode_layer3_frame(this, out, done, synth);
          break;
        default:
    ...
    if (this.free_format) {
      if (this.old_free_format) {
        ...
        /* #if 0
        				Jutil.lame_report_fnc(this.report_dbg,"hip: freeformat bitstream:  estimated bitrate=%dkbs  \n",
  ...
  @SuppressWarnings("boxing")
  public final int decodeMP3(
      final byte[] in, final int isize, final short[] out, final int osize, final int[] done) {
    if (osize < (1152 * 2)) {
   ...
   */
  public final int decodeMP3_unclipped(
      final byte[] in, final int isize, final float[] out, final int osize, final int[] done) {
    ...
    return decodeMP3_clipchoice(
        ...
        sDecoderUnclipped); // synth_1to1_mono_unclipped, synth_1to1_unclipped );
  }
  ...
  final int set_pointer(final int backstep) {
    ...
    if (backstep != 0) {
      ...
      System.arraycopy(
          bsbufold, 512 + this.fsizeold - backstep, this.wordbuf, this.wordpointer, backstep);
    }
    this.bitindex = 0;
    return Jmpg123.MP3_OK;
  }
}

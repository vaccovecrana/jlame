...
class Jdct64 {
  ...
  static final void make_decode_tables(int scaleval) {
    ...
    for (int i = 0; i < 5; i++) {
      ...
      for (int k = 0; k < kr; k++) {
        costab[k] =
            (float) (1.0 / (2.0 * Math.cos(Math.PI * ((double) k * 2.0 + 1.0) / (double) divv)));
      }
    }
  ...
  /**
   * the call via dct64 is a trick to force GCC to use (new) registers for the b1,b2 pointer to the
   * bufs[xx] field
   */
  static final void dct64(
      final float[] out0,
      final int outoffset0,
      final float[] out1,
      final int outoffset1,
      final float[] samples,
      final int soffset) {

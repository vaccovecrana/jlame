...
final class ShoutcastIcecastInputStream extends InputStream {
  ...
  //
  /**
   * Creates buffered stream with the specified buffer size and the metadata period to read
   * SHOUTcast/Icecast data.
   *
   ...
   */
  public ShoutcastIcecastInputStream(final String url, final boolean isReadMetadata, final int size)
      throws IOException {
    ...
    */
    ...
    mInputStream =
        new BufferedInputStream(
            mConnection.getInputStream(), size >= BUFFER_SIZE ? size : BUFFER_SIZE);
  }
  ...
  @Override
  public final synchronized int read(final byte[] b, int off, int len) throws IOException {
    if (mMetadataPeriod == 0) {

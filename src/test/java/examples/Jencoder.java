...
public final class Jencoder {
  ...
  private static final int write_xing_frame(
      final Jlame_global_flags gf, final RandomAccessFile outf, final int offset) {
  ...
  @SuppressWarnings("boxing")
  private static final int write_id3v1_tag(
      final Jlame_global_flags gf, final RandomAccessFile outf) {
  ...
  private static final void setEncodingParameters(
      final Jlame_global_flags gfp, final int vbr_cbr_abr, final float br_param) {
    if (vbr_cbr_abr == VBR) {
    ...
    try {
      ...
      if (in != null) {
        ...
        while ((iread = din.read(mp3buffer)) > 0) {
          ByteBuffer.wrap(mp3buffer, 0, iread)
              .order(ByteOrder.LITTLE_ENDIAN)
              .asShortBuffer()
              .get(pcm, 0, iread >> 1);
          final int imp3 =
              Jlame.lame_encode_buffer_interleaved(
                  gf, pcm, (iread / channels) >> 1, mp3buffer, 0, mp3buffer.length);

...
import io.vacco.libmpghip.Jmpg123;
import io.vacco.libmpghip.Jmpstr_tag;
...
public final class Jdecoder extends Jmpstr_tag {
   ...
   */
  private static final void valueToBytes(
      final int value, final int byteCount, final byte[] data, int offset) {
   ...
   */
  private static final void setSoundFormat(
      final int sampleRate, final int bitsPerSample, final int channels) {
    valueToBytes(channels, 2, PCM_HEADER, 22); // Channels
    valueToBytes(sampleRate, 4, PCM_HEADER, 24); // Sample rate
    valueToBytes(
        sampleRate * channels * (bitsPerSample >> 3), 4, PCM_HEADER, 28); // Avg Bytes per seconds
  ...
  private final boolean isSyncwordMp123(final byte[] header) {
    ...
    if ((header[1] & 0x18) == 0x18
        && (header[1] & 0x06) == 0x04
        && ((sAbl2[((int) header[2] & 0xff) >> 4] & (1 << (((int) header[3] & 0xff) >> 6))) != 0)) {
      return false;
    }
    if ((header[3] & 3) == 2) {
  ...
  /**
   * For lame_decode: return code -1 error 0 ok, but need more data before outputing any samples n
   * number of samples output. either 576 or 1152 depending on MP3 file.
   */
  private final int decodeHeaders(
      final byte[] buffer, final int len, final short[] p, final int psize) {
    final int processed_mono_samples[] =
        new int[1]; // java: processed_bytes changed to processed_mono_samples
    ...
    if (this.header_parsed || this.fsizeold > 0 || this.framesize > 0) {
      ...
      if (this.fsizeold > 0) {
        mBitrate = (int) (8 * (4 + this.fsizeold) * mSampleRate / (1.e3 * this.framesize) + 0.5);
      } else if (this.framesize > 0) {
        mBitrate = (int) (8 * (4 + this.framesize) * mSampleRate / (1.e3 * this.framesize) + 0.5);
      } else {
      ...
      if (decoder.mTotalNumSamples == MAX_U_32_NUM) {
        try {
          ...
          if (decoder.mBitrate > 0) {
            final double totalseconds = (flen * 8.0 / (1000.0 * decoder.mBitrate));
            final long tmp_num_samples = (long) (totalseconds * decoder.mSampleRate);
